...
namespace Ams.CodeGenerator
{
    ...
    public class CodeGeneratorTool
    {
        ...
        public static void Generate(GenerateDto dto)
        {
            ...
            {
                throw new CustomizeException($"{dto.GenTable.SubTableName}子表不存在");
            }

            //string sss = Assembly.GetExecutingAssembly().GetName().Version.ToString();
        ...
        public static string GetClassName(string tableName, bool autoRemovePre, string tablePrefix)
        {
            ...
            //起始截取位置
            int StartIndex = tableName.IndexOf("_") + 1;
            //截取字串长度
            int SubstringLength = tableName.Length - StartIndex;
            //截取字串（去提“_”前面所有字符）
            string ClassName = tableName.Substring(StartIndex, SubstringLength);
            return ClassName.ConvertToPascal("_");
        }
        ...
        public static List<GenTableColumn> InitGenTableColumn(GenTable genTable, List<DbColumnInfo> dbColumnInfos, List<OracleSeq> seqs = null, CodeGen codeGen = null)
        {
            ...
            optionsSetting.CodeGenDbConfig = dbConfig ?? throw new CustomizeException("代码生成节点数据配置异常"); ;
            optionsSetting.CodeGen = codeGen ?? throw new CustomizeException("代码生成节点配置异常");
