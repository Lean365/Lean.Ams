...
using Ams.Kernel.Services.Generate;
...
namespace Ams.CodeGenerator.Service
{
    public class CodeGeneraterService : DbProvider
    {
        ...
        {
            var q = GetSugarDbContext(dbName).DbMaintenance.GetTableInfoList(true);
            var tableList = (from a in q

                             //where a.Name.Contains("fico_") || a.Name.Contains("qm_") || a.Name.Contains("inst_") || a.Name.Contains("mm_") || a.Name.Contains("routine_") || a.Name.Contains("sd_") || a.Name.Contains("pp_")
                             orderby a.Name
                             select a).ToList();

            if (!string.IsNullOrEmpty(tableName))
            {
                tableList = tableList.OrderBy(f => f.Name).Where(f => f.Name.ToLower().Contains(tableName.ToLower())).ToList();
            }
