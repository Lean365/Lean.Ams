...
namespace Ams.Generator
{
    ...
    public class GenTool
    {
        ...
        public static void Generator(GenDto dto)
        {
            ...
            dto.GenOptions = GenOption(dto.GenTable);
            if (dto.GenTable.SubTable != null)
            {
                dto.SubTableOptions = GenOption(dto.GenTable.SubTable);
            }
            ...
            GenReplaceDto _GenReplaceDto = new()
            {
                //程序版本
            ...
            _GenReplaceDto.PKName = columns.Find(f => f.IsPk || f.IsIncrement)?.CsharpField ?? "Id";
            _GenReplaceDto.PKType = columns.Find(f => f.IsPk || f.IsIncrement)?.CsharpType ?? "int";

            _GenReplaceDto.UploadFile = columns.Any(f => f.HtmlType.Equals(GenConstants.HTML_IMAGE_UPLOAD) || f.HtmlType.Equals(GenConstants.HTML_FILE_UPLOAD)) ? 1 : 0;
            _GenReplaceDto.SelectMulti = columns.Any(f => f.HtmlType.Equals(GenConstants.HTML_SELECT_MULTI)) ? 1 : 0;
            _GenReplaceDto.SelectRemote = columns.Any(f => f.HtmlType.Equals(GenConstants.HTML_SELECT_REMOTE)) ? 1 : 0;
            _GenReplaceDto.ShowEditor = columns.Any(f => f.HtmlType.Equals(GenConstants.HTML_EDITOR)) ? 1 : 0;
            _GenReplaceDto.FistLowerPk = _GenReplaceDto.PKName.FirstLowerCase();
            InitJntTemplate(dto, _GenReplaceDto);

            GeneratorModels(_GenReplaceDto, dto);
            GeneratorService(_GenReplaceDto, dto);
            GeneratorControllers(_GenReplaceDto, dto);
            if (dto.GenTable.Options.FrontTpl == 2)
            {
                GeneratorVue3Views(_GenReplaceDto, dto);
            }
            else if (dto.GenTable.Options.FrontTpl == 1)
            {
                GeneratorVueViews(_GenReplaceDto, dto);
            }
            else
            {
                GeneratorAntDesignViews(_GenReplaceDto, dto);
            }
            if (dto.GenTable.Options.GeneratorRepo == 1)
            {
                GeneratorRepository(_GenReplaceDto, dto);
            }
            GeneratorVueJs(dto);
            GeneratorSql(dto);
            if (genOptions.ShowApp)
            {
                GeneratorAppVueViews(_GenReplaceDto, dto);
                GeneratorAppVueFormViews(_GenReplaceDto, dto);
                GeneratorAppJs(dto);
            }
            dto.ReplaceDto = _GenReplaceDto;
        }

        private static GenOption GenOption(GenTable genTable)
        {
            GenOption _option = new()
            {
        ...
        /// 生成实体类Model
        /// </summary>
        /// <param name="GenDto"></param>
        /// <param name="replaceDto">替换实体</param>
        private static void GeneratorModels(GenReplaceDto replaceDto, GenDto GenDto)
        {
            ...
            string fullPath = Path.Combine(GenDto.GenOptions.ModelsNamespace, "Models", GenDto.GenOptions.SubNamespace, replaceDto.ModelTypeName + ".cs");
            string fullPathDto = Path.Combine(GenDto.GenOptions.ModelsNamespace, "Dto", GenDto.GenOptions.SubNamespace, replaceDto.ModelTypeName + "Dto.cs");

            GenDto.GenCodes.Add(new GenCode(1, "Model.cs", fullPath, tpl.Render()));
            GenDto.GenCodes.Add(new GenCode(2, "Dto.cs", fullPathDto, tplDto.Render()));
        }
        ...
        /// <param name="GenDto"></param>
        /// <param name="replaceDto">替换实体</param>
        private static void GeneratorRepository(GenReplaceDto replaceDto, GenDto GenDto)
        {
            var tpl = JnHelper.ReadTemplate(Path.Combine(CodeTemplateDir, "csharp"), "TplRepository.txt");
            var result = tpl.Render();
            var fullPath = Path.Combine(GenDto.GenOptions.RepositoriesNamespace, GenDto.GenOptions.SubNamespace, $"{replaceDto.ModelTypeName}Repository.cs");

            GenDto.GenCodes.Add(new GenCode(3, "Repository.cs", fullPath, result));
        }

        /// <summary>
        /// 生成Service文件
        /// </summary>
        private static void GeneratorService(GenReplaceDto replaceDto, GenDto GenDto)
        {
            var path = Path.Combine(CodeTemplateDir, "csharp");
            var tpl = JnHelper.ReadTemplate(path, "TplService.txt");
            var tpl2 = JnHelper.ReadTemplate(path, "TplIService.txt");

            var fullPath = Path.Combine(GenDto.GenOptions.ServicesNamespace, GenDto.GenOptions.SubNamespace, $"{replaceDto.ModelTypeName}Service.cs");
            var fullPath2 = Path.Combine(GenDto.GenOptions.IServicsNamespace, GenDto.GenOptions.SubNamespace, $"I{GenDto.GenOptions.SubNamespace}Service", $"I{replaceDto.ModelTypeName}Service.cs");

            GenDto.GenCodes.Add(new GenCode(4, "Service.cs", fullPath, tpl.Render()));
            GenDto.GenCodes.Add(new GenCode(4, "IService.cs", fullPath2, tpl2.Render()));
        }

        /// <summary>
        /// 生成控制器ApiControllers文件
        /// </summary>
        private static void GeneratorControllers(GenReplaceDto replaceDto, GenDto GenDto)
        {
            ...
            var fullPath = Path.Combine(GenDto.GenOptions.ApiControllerNamespace, "Controllers", GenDto.GenOptions.SubNamespace, $"{replaceDto.ModelTypeName}Controller.cs");
            GenDto.GenCodes.Add(new GenCode(5, "Controller.cs", fullPath, result));
        }

        /// <summary>
        /// 生成Vue页面
        private static void GeneratorVueViews(GenReplaceDto replaceDto, GenDto GenDto)
        {
            string fileName = GenDto.GenTable.TplCategory switch
            {
                "tree" => "TplTreeVue.txt",
            ...
            var fullPath = Path.Combine(GenDto.VueParentPath, "src", "views", GenDto.GenTable.ModuleName.FirstLowerCase(), $"{replaceDto.ViewFileName}.vue");

            GenDto.GenCodes.Add(new GenCode(6, "index.vue", fullPath, tpl.Render()));
        }
        ...
        /// </summary>
        /// <param name="GenDto"></param>
        private static void GeneratorVue3Views(GenReplaceDto replaceDto, GenDto GenDto)
        {
            string fileName = GenDto.GenTable.TplCategory switch
            {
                "tree" => "TreeVue.txt",
            ...
            var tpl = JnHelper.ReadTemplate(CodeTemplateDir, fileName);
            tpl.Set("treeCode", GenDto.GenTable?.Options?.TreeCode?.FirstLowerCase());
            tpl.Set("treeName", GenDto.GenTable?.Options?.TreeName?.FirstLowerCase());
            tpl.Set("treeParentCode", GenDto.GenTable?.Options?.TreeParentCode?.FirstLowerCase());
            tpl.Set("options", GenDto.GenTable?.Options);

            var result = tpl.Render();
            var fullPath = Path.Combine(GenDto.VueParentPath, "src", "views", GenDto.GenTable.ModuleName.FirstLowerCase(), $"{replaceDto.ViewFileName}.vue");
            GenDto.GenCodes.Add(new GenCode(16, "index.vue", fullPath, result));
        }
        ...
        /// </summary>
        /// <param name="GenDto"></param>
        private static void GeneratorAntDesignViews(GenReplaceDto replaceDto, GenDto GenDto)
        {
            string fileName = GenDto.GenTable.TplCategory switch
            {
                "tree" => "TreeVue.txt",
        ...
        /// <param name="GenDto"></param>
        /// <returns></returns>
        public static void GeneratorVueJs(GenDto GenDto)
        {
            var tpl = JnHelper.ReadTemplate(CodeTemplateDir, "TplVueApi.txt");
            var result = tpl.Render();

            string fileName;
            if (GenDto.VueVersion == 3)
            {
                fileName = GenDto.GenTable.BusinessName.ToLower() + ".js";//vue3 api引用目前只能小写
            }
            else
            {
                fileName = GenDto.GenTable.BusinessName.FirstLowerCase() + ".js";
            }
            string fullPath = Path.Combine(GenDto.VueParentPath, "src", "api", GenDto.GenTable.ModuleName.FirstLowerCase(), fileName);

            GenDto.GenCodes.Add(new GenCode(7, "api.js", fullPath, result));
        }
        ...
        /// </summary>
        /// <param name="GenDto"></param>
        public static void GeneratorSql(GenDto GenDto)
        {
            string tempName = GenDto.DbType switch
            {
                0 => "MySqlTemplate",
                1 => "SqlTemplate",
                4 => "PgSql",
            ...
            tpl.Set("parentId", GenDto.GenTable?.Options?.ParentMenuId ?? 0);
            var result = tpl.Render();
            string fullPath = Path.Combine(GenDto.GenCodePath, "sql", GenDto.GenTable.BusinessName + ".sql");

            GenDto.GenCodes.Add(new GenCode(8, "sql菜单", fullPath, result));
        }
        ...
        /// </summary>
        /// <param name="GenDto"></param>
        private static void GeneratorAppVueViews(GenReplaceDto replaceDto, GenDto GenDto)
        {
            ...
            var result = tpl.Render();
            var fullPath = Path.Combine(GenDto.AppVuePath, "pages", GenDto.GenTable.ModuleName.FirstLowerCase(), $"{replaceDto.ViewFileName.FirstLowerCase()}", "index.vue");
            GenDto.GenCodes.Add(new GenCode(20, "uniapp页面", fullPath, result));
        }

        private static void GeneratorAppVueFormViews(GenReplaceDto replaceDto, GenDto GenDto)
        {
            ...
            var result = tpl.Render();
            var fullPath = Path.Combine(GenDto.AppVuePath, "pages", GenDto.GenTable.ModuleName.FirstLowerCase(), $"{replaceDto.ViewFileName.FirstLowerCase()}", "edit.vue");
            GenDto.GenCodes.Add(new GenCode(20, "uniapp表单", fullPath, result));
        }
        ...
        /// <param name="GenDto"></param>
        /// <returns></returns>
        public static void GeneratorAppJs(GenDto GenDto)
        {
            var filePath = Path.Combine("app", "api.txt");
            var tpl = JnHelper.ReadTemplate(CodeTemplateDir, filePath);
            var result = tpl.Render();

            string fileName = GenDto.GenTable.BusinessName.ToLower() + ".js";
            string fullPath = Path.Combine(GenDto.AppVuePath, "api", GenDto.GenTable.ModuleName.FirstLowerCase(), fileName);

            GenDto.GenCodes.Add(new GenCode(21, "uniapp Api", fullPath, result));
        }
        ...
        /// <param name="dto"></param>
        /// <returns></returns>
        public static GenTable InitTable(GenInitTableDto dto)
        {
            //应用程序启动目录
        ...
        public static List<GenTableColumn> InitGenTableColumn(GenTable genTable, List<DbColumnInfo> dbColumnInfos, List<GenOracleSeq> seqs = null, CodeGen codeGen = null)
        {
        ...
        /// <returns></returns>
        private static GenTableColumn InitColumnField(GenTable genTable, DbColumnInfo column, List<GenOracleSeq> seqs, OptionsSetting optionsSetting)
        {
            ...
            {
                genTableColumn.HtmlType = GenConstants.HTML_INPUT_NUMBER;
            }
            //时间类型初始化between范围查询
        ...
        private static void InitJntTemplate(GenDto dto, GenReplaceDto replaceDto)
        {
#if DEBUG
            ...
            var dictHtml = new string[] { GenConstants.HTML_CHECKBOX, GenConstants.HTML_RADIO, GenConstants.HTML_SELECT, GenConstants.HTML_SELECT_MULTI, GenConstants.HTML_SELECT_REMOTE };
            ...
            Engine.Configure((options) =>
            {
                ...
                options.Data.Set("tool", new GenTool());
                options.Data.Set("codeTool", new GenTemplate());
