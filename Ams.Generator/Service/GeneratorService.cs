...
namespace Ams.Generator.Service
{
    public class GeneratorService : GeneratorDbProvider
    {
        /// <summary>
        /// 获取所有数据库名
        /// </summary>
        ...
        public List<string> GetAllDataBases()
        {
            ...
            //读取配置文件中数据库名称
            string connDbName = AppSettings.GetConfig(GeneratorConstants.Gen_dbname);
            var db = GetSugarDbContext();
            ////Oracle库特殊处理
            //DbConfigs configs = AppSettings.Get<DbConfigs>(nameof(GlobalConstant.CodeGenDbConfig));
            //if (configs.DbType == 3)
            ...
            //return templist.FindAll(f => !f.Contains("schema"));
            //Oracle库特殊处理
            DbConfigs configs = AppSettings.Get<DbConfigs>(nameof(GeneratorConstants.CodeGenDbConfig));
            if (configs.DbType == 3)
            {
                return new List<string>() { configs?.DbName };
            }
        ...
        /// <param name="dbName"></param>
        /// <returns></returns>
        public List<GeneratorOracleSeq> GetAllGeneratorOracleSeqs(string dbName)
        {
            string sql = "SELECT * FROM USER_SEQUENCES";
            var seqs = GetSugarDbContext(dbName).Ado.SqlQuery<GeneratorOracleSeq>(sql);
