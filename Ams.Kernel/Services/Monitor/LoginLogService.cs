...
namespace Ams.Kernel.Services.Monitor
{
    ...
    /// @Date 2024-01-01
    /// </summary>
    [AppService(ServiceType = typeof(ILoginLogService), ServiceLifetime = LifeTime.Transient)]
    public class LoginLogService : BaseService<LoginLog>, ILoginLogService
    {
        private readonly ISysUserService SysUserService;
        private readonly IHttpContextAccessor httpContextAccessor;

        public LoginLogService(ISysUserService sysUserService, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public SysUser Login(LoginBodyDto loginBody, LoginLog loginlog)
        {
            ...
            loginlog.Browser = clientInfo.ToString();
            loginlog.Os = clientInfo.OS.ToString();

            if (user == null || user.UserId <= 0)
            {
                loginlog.Msg = "用户名或密码错误";
                AddLoginInfo(loginlog);
        ...
        /// <returns></returns>
        public SysUser PhoneLogin(PhoneLoginDto loginBody, LoginLog loginlog, SysUser user)
        {
        ...
        public PagedInfo<LoginLog> GetLoginLog(LoginLog logininfoDto, PagerInfo pager)
        {
            ...
            exp.AndIF(logininfoDto.IsStated.ToString().IfNotEmpty(), f => f.IsStated == logininfoDto.IsStated);
