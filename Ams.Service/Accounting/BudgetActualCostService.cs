...
namespace Ams.Service.Accounting
{
    ...
    /// </summary>
    [AppService(ServiceType = typeof(IBudgetActualCostService), ServiceLifetime = LifeTime.Transient)]
    public class BudgetActualCostService : BaseService<BudgetActualCost>, IBudgetActualCostService
    {
        /// <summary>
        /// 查询预算实际明细列表
        /// </summary>
        ...
        /// <returns></returns>
        public PagedInfo<BudgetActualCostDto> GetList(BudgetActualCostQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .ToPage<BudgetActualCost, BudgetActualCostDto>(parm);
        ...
        /// <returns></returns>
        public BudgetActualCost GetInfo(long FbSFID)
        {
            var response = Queryable()
        ...
        /// <returns></returns>
        public BudgetActualCost AddBudgetActualCost(BudgetActualCost model)
        {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int UpdateBudgetActualCost(BudgetActualCost model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public (string, object, object) ImportBudgetActualCost(List<BudgetActualCost> list)
        {
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public PagedInfo<BudgetActualCostDto> ExportList(BudgetActualCostQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .Select((it) => new BudgetActualCostDto()
                {
                    FbFyLabel = it.FbFy.GetConfigValue<Kernel.Model.System.SysDictData>("sql_period_list"),
                    FbYmLabel = it.FbYm.GetConfigValue<Kernel.Model.System.SysDictData>("sql_ficoym_list"),
                    FbCorpCodeLabel = it.FbCorpCode.GetConfigValue<Kernel.Model.System.SysDictData>("sys_crop_list"),
                }, true)
                .ToPage(parm);
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        private static Expressionable<BudgetActualCost> QueryExp(BudgetActualCostQueryDto parm)
        {
            var predicate = Expressionable.Create<BudgetActualCost>();
