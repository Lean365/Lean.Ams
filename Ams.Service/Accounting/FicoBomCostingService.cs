...
namespace Ams.Service.Accounting
{
    ...
    public class FicoBomCostingService : BaseService<FicoBomCosting>, IFicoBomCostingService
    {
        ...
        /// <returns></returns>
        public FicoBomCosting GetInfo(long BcSFID)
        {
            var response = Queryable()
        ...
        /// <returns></returns>
        public FicoBomCosting AddFicoBomCosting(FicoBomCosting model)
        {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int UpdateFicoBomCosting(FicoBomCosting model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public (string, object, object) ImportFicoBomCosting(List<FicoBomCosting> list)
        {
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public PagedInfo<FicoBomCostingDto> ExportList(FicoBomCostingQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .Select((it) => new FicoBomCostingDto()
                {
                    BcPlantLabel = it.BcPlant.GetConfigValue<SysDictData>("sys_plant_list"),
                    BcYmLabel = it.BcYm.GetConfigValue<SysDictData>("sql_ym_list"),
                    BcCurrencyLabel = it.BcCurrency.GetConfigValue<SysDictData>("sys_ccy_type"),
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        private static Expressionable<FicoBomCosting> QueryExp(FicoBomCostingQueryDto parm)
        {
            ...
            //predicate = predicate.AndIF(parm.BeginBcBalancedate == null, it => it.BcBalancedate >= DateTime.Now.ToShortDateString().ParseToDateTime());
            //当日期条件为空时，默认查询大于今年的所有数据
            predicate = predicate.AndIF(parm.BeginBcBalancedate == null, it => it.BcBalancedate >= new DateTime(DateTime.Now.Year, 1, 1));
            predicate = predicate.AndIF(parm.BeginBcBalancedate != null, it => it.BcBalancedate >= parm.BeginBcBalancedate);
            predicate = predicate.AndIF(parm.EndBcBalancedate != null, it => it.BcBalancedate <= parm.EndBcBalancedate);
            return predicate;
        }
    }
}