    ...
    public class SysUserService : BaseService<SysUser>, ISysUserService
    {
        ...
        public SysUser SelectUserById(long userId)
        {
            var user = Queryable().Filter(null, true).WithCache(60 * 5)
                .Where(f => f.UserId == userId && f.Is_deleted == 0).First();
        ...
        {
            int count = Count(it => it.UserName == userName && it.Is_deleted == 0);
            if (count > 0)
            {
        ...
        public int DeleteUser(long userid)
        {
            ...
            var result = UseTran(() =>
            {
                ...
                Update(new SysUser() { UserId = userid, Is_deleted = 2 }, it => new { it.Is_deleted }, f => f.UserId == userid);
            });
            return result.IsSuccess ? 1 : 0;
        }
        ...
        {
            return GetFirst(it => it.UserName == user.Username && it.Password.ToLower() == user.Password.ToLower() && it.Is_deleted == 0);
        }
