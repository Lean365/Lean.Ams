...
namespace Ams.Service.Content
{
    ...
    public class ArticleCommentService : BaseService<ArticleComment>, IArticleCommentService
    {
        ...
        private IEmailSentItemsService _EmailSentItemsService;
        ...
        {
            var configInfo = _SysConfigService.GetFirst(f => f.ConfigKey == "article.comment") ?? throw new CustomException(ResultCode.CUSTOM_ERROR, "评论失败，请联系管理员");
            var userInfo = _SysUserService.GetById(message.UserId);
            ...
            var r = UseTran(() =>
            {
                ...
                if (result != null && result.CommentId > 0)
                {
                    ...
                    {
                        _ArticleService.Update(it => it.Cid == (int)message.TargetId, it => new Article() { CommentNum = it.CommentNum + 1 });
                    }
                }
            });
            //查询出评论的内容信息找出作者
        ...
        public int DeleteMessage(long commentId, long userId)
        {
            ...
            var postInfo = _ArticleService.GetById(info.TargetId);
            ...
            var result = UseTran(() =>
            {
                ...
                if (info.TargetId > 0)
                {
                    ...
                    _ArticleService.Update(it => it.Cid == (int)info.TargetId, it => new Article()
                    {
