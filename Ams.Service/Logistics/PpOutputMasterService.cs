    ...
    public class PpOutputMasterService : BaseService<PpOutputMaster>, IPpOutputMasterService
    {
        ...
        public PagedInfo<PpOutputMasterDto> GetList(PpOutputMasterQueryDto parm)
        {
            ...
            var response = Queryable()
                //.Includes(x => x.PpOutputSlaveNav) //填充子对象
        ...
        /// <returns></returns>
        public PpOutputMaster GetInfo(long PomSfid)
        {
            var response = Queryable()
                .Includes(x => x.PpOutputSlaveNav) //填充子对象
        ...
        /// <returns></returns>
        public PpOutputMaster AddPpOutputMaster(PpOutputMaster model)
        {
            return Context.InsertNav(model).Include(s1 => s1.PpOutputSlaveNav).ExecuteReturnEntity();
        }
        ...
        {
            return Context.UpdateNav(model).Include(z1 => z1.PpOutputSlaveNav).ExecuteCommand() ? 1 : 0;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public (string, object, object) ImportPpOutputMaster(List<PpOutputMaster> list)
        {
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public PagedInfo<PpOutputMasterDto> ExportList(PpOutputMasterQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .Select((it) => new PpOutputMasterDto()
                {
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        private static Expressionable<PpOutputMaster> QueryExp(PpOutputMasterQueryDto parm)
        {
            var predicate = Expressionable.Create<PpOutputMaster>();

            predicate = predicate.AndIF(!string.IsNullOrEmpty(parm.PomMfgOrder), it => it.PomMfgOrder.Contains(parm.PomMfgOrder));
            ...
            predicate = predicate.AndIF(parm.BeginPomMfgDate == null, it => it.PomMfgDate >= new DateTime(DateTime.Now.Year, 1, 1));
            ...
            predicate = predicate.AndIF(!string.IsNullOrEmpty(parm.PomMfgLine), it => it.PomMfgLine.Contains(parm.PomMfgLine));
            return predicate;
        }
    }
}