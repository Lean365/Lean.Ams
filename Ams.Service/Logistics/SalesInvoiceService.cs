...
namespace Ams.Service.Logistics
{
    ...
    /// </summary>
    [AppService(ServiceType = typeof(ISalesInvoiceService), ServiceLifetime = LifeTime.Transient)]
    public class SalesInvoiceService : BaseService<SalesInvoice>, ISalesInvoiceService
    {
        ...
        /// <returns></returns>
        public PagedInfo<SalesInvoiceDto> GetList(SalesInvoiceQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .ToPage<SalesInvoice, SalesInvoiceDto>(parm);
        ...
        /// <returns></returns>
        public SalesInvoice GetInfo(long SsiSfid)
        {
            var response = Queryable()
        ...
        /// <returns></returns>
        public SalesInvoice AddSalesInvoice(SalesInvoice model)
        {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int UpdateSalesInvoice(SalesInvoice model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public (string, object, object) ImportSalesInvoice(List<SalesInvoice> list)
        {
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public PagedInfo<SalesInvoiceDto> ExportList(SalesInvoiceQueryDto parm)
        {
            var predicate = QueryExp(parm);

            var response = Queryable()
                .Where(predicate.ToExpression())
                .Select((it) => new SalesInvoiceDto()
                {
                    SsiPlntLabel = it.SsiPlnt.GetConfigValue<Kernel.Model.System.SysDictData>("app_plant_list"),
                    SsiFyLabel = it.SsiFy.GetConfigValue<Kernel.Model.System.SysDictData>("sql_period_list"),
                    SsiYmLabel = it.SsiYm.GetConfigValue<Kernel.Model.System.SysDictData>("sql_ficoym_list"),
                    SsiPrctrLabel = it.SsiPrctr.GetConfigValue<Kernel.Model.System.SysDictData>("sql_prctr_list"),
                    SsiOriginalccyLabel = it.SsiOriginalccy.GetConfigValue<Kernel.Model.System.SysDictData>("sys_ccy_type"),
                }, true)
                .ToPage(parm);
