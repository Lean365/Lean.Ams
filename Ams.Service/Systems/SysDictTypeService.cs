...
namespace Ams.Service.Systems
{
    ...
    public class SysDictTypeService : BaseService<SysDictType>, ISysDictTypeService
    {
        ...
        public PagedInfo<SysDictType> SelectDictTypeList(SysDictType dictType, PagerInfo pager)
        {
            ...
            exp.AndIF(dictType.IsStatus != -1, u => u.IsStatus == dictType.IsStatus);
            exp.AndIF(dictType.DictCategory != -1, u => u.DictCategory == dictType.DictCategory);
            exp.AndIF(dictType.Type != "A", it => it.Type.Equals(dictType.Type));
