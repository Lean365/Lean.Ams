    ...
    public class InventoryStatiService : BaseService<FicoMonthlyInventory>, IInventoryStatiService
    {
        ...
        public decimal GetMonthlyInventoryAmount(FicoMonthlyInventoryQueryDto parm)
        {
            ...
            var response = Queryable()
                ...
                .Select((x) => new
                {
                    MiInventoryAmount = SqlFunc.AggregateSumNoNull(SqlFunc.IIF(x.MiInventoryAmount > 0, x.MiInventoryAmount / 10000, 0)), //求和,
            ...
            {
                MonthlyInventoryAmount = response[0].MiInventoryAmount;
            }
        ...
        private static Expressionable<FicoMonthlyInventory> QueryExp(FicoMonthlyInventoryQueryDto parm)
        {
            ...
            if (parm.MiYm == null)
            {
                predicate = predicate.AndIF(true, it => it.MiYm == "202312");
            }
            else
            {
                predicate = predicate.AndIF(parm.MiYm != null, it => it.MiYm == parm.MiYm);
            }
