    ...
    <div class="login">
      ...
      <el-form ref="loginRef" :model="loginForm" :rules="loginRules" class="login-form" v-show="loginType == 1">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center">
          ...
          <span style="font-size: 12px">
            <router-link class="link-type" :to="'/register'">{{ $t('plogin.register') }}</router-link>
      ...
      <div class="qr-wrap login-form" v-show="loginType == 3">
        <div class="login-scan-container">
          ...
          <div class="mt10 text-muted">{{ $t('plogin.uniappscan') }}</div>
        </div>
      </div>
...
<script setup name="login">
  ...
  const loginRules = {
    ...
    code: [{ required: true, trigger: 'change', message: proxy.$t('btn.enter') + proxy.$t('plogin.smscode') }]
  }
  ...
  function formatInput(value) {
    if (value) {
      ...
      loginForm.username = event.target.value.toLowerCase().replace(/[^A-Z0-9]/g, '');
      console.log(value.toLowerCase())
    }
  }
  const formattedValue = computed(() => loginForm.username.toLowerCase());
  ...
  function generateCode() {
    ...
    document.getElementById('imgContainer').innerHTML = proxy.$t('plogin.qrcodeGenerating')
    ...
    interval.value = setInterval(() => {
      verifyScan({ uuid: uuid, deviceId: userStore.clientId })
        .then((res) => {
          ...
          if (data.status == -1) {
            clearQr()
            document.getElementById('imgContainer').innerHTML = proxy.$t('plogin.qrcodeExpired')
