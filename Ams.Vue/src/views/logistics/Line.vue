  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="班组类别" prop="plLineType">
        <el-select clearable filterable v-model="queryParams.plLineType" :placeholder="$t('btn.select')+'班组类别'">
          <el-option v-for="item in  options.app_line_type " :key="item.dictValue" :label="item.dictLabel"
    ...
    <el-row :gutter="15" class="mb10">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['pp:line:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['pp:line:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData templateUrl="logistics/Line/importTemplate" importUrl="/logistics/Line/importData"
    ...
    <el-table :data="dataList" height="650" v-loading="loading" ref="table" border
      ...
      <el-table-column prop="plLineLangCode" label="语言Key" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('plLineLangCode')" />
      ...
      <el-table-column :label="$t('btn.operation')" width="150" align="center">
        <template #default="scope">
          <el-button type="success" size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:line:edit']"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button type="danger" size="small" plain icon="delete" :title="$t('btn.delete')"
            v-hasPermi="['pp:line:delete']" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
          <el-tab-pane :label="$t('ptabs.basicInfo')" name="first">
            ...
            <el-form-item label="SFID" prop="plSFID">
              ...
              <el-col :lg="12">
                <el-form-item label="班组类别" prop="plLineType">
                  <el-select clearable filterable v-model="form.plLineType" :placeholder="$t('btn.select')+'班组类别'"
                    @change="assignValue($event)">
                    <el-option v-for="item in options.app_line_type" :key="item.dictValue" :label="item.dictLabel"
              ...
              <el-col :lg="12">
                <el-form-item label="班组代码" prop="plLineCode">
                  <el-input v-model="form.plLineCode" :placeholder="$t('btn.enter')+'班组代码'" maxlength="6"
                    show-word-limit @input="assignValue($event)" />
                </el-form-item>
              </el-col>

              <el-col :lg="12">
                <el-form-item label="语言Key" prop="plLineLangCode">
                  <el-input v-model="form.plLineLangCode" :placeholder="$t('btn.enter')+'语言Key'" :disabled="true" />
                </el-form-item>
              </el-col>

              <el-col :lg="12">
                <el-form-item label="班组名称" prop="plLineName">
                  <el-input v-model="form.plLineName" :placeholder="$t('btn.enter')+'班组名称'" maxlength="10"
                    show-word-limit @input="assignValue($event)" />
                </el-form-item>
              </el-col>
...
<script setup name="line">
  ...
  const state = reactive({
    ...
    form: {},
    rules: {
      plSFID: [{ required: true, message: 'SFID' + proxy.$t('btn.empty'), trigger: "blur" }],
      ...
      {
        min: 1, max: 6, message: '班组代码：最小1个字符长度，最大6位。', trigger: "blur"

      }],
      plLineLangCode: [{ required: true, message: '语言Key' + proxy.$t('btn.empty'), trigger: "blur" }],
      plLineName: [{ required: true, message: '班组名称' + proxy.$t('btn.empty'), trigger: "blur" }, {
        min: 3, max: 10, message: '班组代码：最小3个字符长度，最大10位。', trigger: "blur"
      }],
    },
    options: {
  ...
  function assignValue(val) {
    if (proxy.strIsNullOrEmpty(form.value.plLineType) || proxy.strIsNullOrEmpty(form.value.plLineCode) || proxy.strIsNullOrEmpty(form.value.plLineName)) {

      form.value.plLineLangCode = ('dict.app_line_' + form.value.plLineType + '.' + form.value.plLineCode + '.' + form.value.plLineName).toLowerCase()

    }
    else {

      form.value.plLineLangCode = ('dict.app_line_' + form.value.plLineType + '.' + pinyin.getCamelChars(form.value.plLineCode) + '.' + pinyin.getCamelChars(form.value.plLineName)).toLowerCase()
  ...
  function handleExport() {
    proxy
      ...
      .then(async () => {
        await proxy.downFile('/logistics/Line/export', { ...queryParams })
      })
  }
