  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="生产工厂" prop="moPlant">
        <el-select clearable filterable v-model="queryParams.moPlant" :placeholder="$t('btn.select')+'生产工厂'">
      ...
      <el-form-item label="订单类型" prop="moOrderType">
        <el-select clearable filterable v-model="queryParams.moOrderType" :placeholder="$t('btn.select')+'订单类型'">
      ...
      <el-form-item label="物料" prop="moOrderItem">
        <el-select filterable remote remote-show-suffix :remote-method="remoteMethod" clearable :loading="loading"
          ...
          <el-option v-for="item in  remotequery_sql_moc_item" :key="item.dictValue" :label="item.dictLabel"
            :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>
          </el-option>
      ...
      <el-form-item label="状态" prop="isState">
        <el-radio-group v-model="queryParams.isState">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData templateUrl="logistics/Order/importTemplate" importUrl="/logistics/Order/importData"
    ...
    <el-table :data="dataList" height="650" v-loading="loading" ref="table" border
      ...
      <el-table-column prop="isState" label="状态" align="center" v-if="columns.showColumn('isState')">
        <template #default="scope">
          <dict-tag :options=" options.sys_is_status " :value="scope.row.isState" />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operation')" width="150" align="center">
        <template #default="scope">
          <el-button type="success" size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:order:edit']"
        ...
        <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
          <el-tab-pane :label="$t('ptabs.basicInfo')" name="first">
            <el-row :gutter="20">
              ...
              <el-col :lg="12">
                <el-form-item label="生产工厂" prop="moPlant">
                  <el-select clearable filterable v-model="form.moPlant" :placeholder="$t('btn.select')+'生产工厂'">
                    <el-option v-for="item in options.app_plant_list" :key="item.dictValue" :label="item.dictLabel"
              ...
              <el-col :lg="12">
                <el-form-item label="订单类型" prop="moOrderType">
                  <el-select clearable filterable v-model="form.moOrderType" :placeholder="$t('btn.select')+'订单类型'"
                    @change="assignOrderNumValue">
                    <el-option v-for="item in options.app_mo_type" :key="item.dictValue" :label="item.dictLabel"
                      :value="item.dictValue"></el-option>
                  </el-select>
                </el-form-item>
              ...
              <el-col :lg="12">
                <el-form-item label="生产订单" prop="moOrderNo">
                  <el-input v-model="form.moOrderNo" :placeholder="$t('btn.enter')+'生产订单'" :disabled="true">
                    <template #prepend>自动生成</template>
                  </el-input>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="物料" prop="moOrderItem">
                  <el-select filterable remote remote-show-suffix :remote-method="remoteMethod" clearable
                    :loading="loading" style="width: 360px" v-model="form.moOrderItem"
              ...
              <el-col :lg="12">
                <el-form-item label="批次" prop="moOrderlot">
                  <el-input v-model="form.moOrderlot" :placeholder="$t('btn.enter')+'批次'" maxlength="10"
                    show-word-limit />
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="工艺路线" prop="moOrderRoute">
                  <el-input v-model="form.moOrderRoute" :placeholder="$t('btn.enter')+'工艺路线'" maxlength="8"
                    show-word-limit />
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="序列号" prop="moOrderSerial">
                  <el-input v-model="form.moOrderSerial" :placeholder="$t('btn.enter')+'序列号'" maxlength="40"
                    show-word-limit />
                </el-form-item>
              </el-col>
...
<script setup name="order">
  ...
  //字段显示控制
  const columns = ref([
    { visible: false, prop: 'moSFID', label: 'SFID' },
    ...
    { visible: true, prop: 'moOrderSerial', label: '序列号' },
    { visible: false, prop: 'isState', label: '状态' },
  ...
  const state = reactive({
    ...
    form: {},
    rules: {
      moSFID: [{ required: true, message: 'SFID' + proxy.$t('btn.empty'), trigger: "blur" }],
      ...
      isState: [{ required: true, message: '状态' + proxy.$t('btn.empty'), trigger: "blur", type: "number" }],
    },
    options: {
  ...
  // @Author: (Lean365)
  // @Date: (2024/1/24 9:49:40)
  function assignOrderNumValue(val) {

    //form.value.moOrderNo = GetOrderNum(val)
    queryParams.moOrderType = val
    getOrderNum(queryParams).then(res => {
      const { code, data } = res
      if (code == 200) {
        form.value.moOrderNo = data
      }
    })
  }
  ...
  function handleExport() {
    proxy
      ...
      .then(async () => {
        await proxy.downFile('/logistics/Order/export', { ...queryParams })
      })
  }
