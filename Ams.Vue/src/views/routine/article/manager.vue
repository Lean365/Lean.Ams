<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="plus" v-hasPermi="['routine:article:add']"
      ...
      <el-table-column label="操作" align="center" width="130" fixed="right">
        <template #default="scope">
          ...
          <el-button text size="small" icon="edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['routine:article:update']">编辑</el-button>
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row)" style="margin-left: 10px">
            <template #reference>
              <el-button text size="small" icon="delete" v-hasPermi="['routine:article:delete']">删除</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
...
<script setup name="index">
  import { listArticle, delArticle, topArticle, changeArticlePublic } from '@/api/routine/article.js'
  ...
  function handleAdd() {
    ...
    router.push({ path: '/routine/publish' })
  }
  ...
  function handleUpdate(row) {
    router.push({ path: '/routine/publish', query: { cid: row.cid } })
  }
  // 详情
