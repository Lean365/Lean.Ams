  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" label-position="left" inline ref="queryForm" v-show="showSearch" @submit.prevent>
      <el-form-item label="" prop="fileId">
        <el-input v-model="queryParams.fileId" :placeholder="$t('fupload.fileID')" clearable />
      </el-form-item>
      <el-form-item label="">
        <el-date-picker v-model="dateRangeAddTime" type="daterange" range-separator="-"
          ...
          :placeholder="$t('btn.select')+$t('fupload.fileCreatedate')"></el-date-picker>
      </el-form-item>
      <el-form-item label="" prop="storeType">
        <el-radio-group v-model="queryParams.storeType" @change="handleQuery" :placeholder="$t('fupload.storageType')">
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row
      ...
      <el-table-column prop="id" :label="$t('fupload.fileID')" align="center" width="180" />
      <el-table-column prop="fileName" :label="$t('fupload.fileDown')" align="left" width="180"
        :show-overflow-tooltip="true">
        <template #default="scope">
          <el-link type="primary" :href="scope.row.accessUrl" target="_blank">{{ scope.row.fileName }}</el-link>
        </template>
      </el-table-column>
      <el-table-column prop="accessUrl" align="center" :label="$t('fupload.filePreview')" width="120">
      ...
      <el-table-column prop="storePath" :label="$t('fupload.viewFilestorage')"></el-table-column>
      <el-table-column prop="createBy" :label="$t('fupload.fileOperator')" align="center" />
      <el-table-column prop="createTime" :label="$t('fupload.fileCreatedate')" align="center" width="150" />
      <el-table-column :label="$t('fupload.fileOperation')" align="center" width="160">
        <template #default="scope">
          ...
          <el-col :lg="24">
            <el-form-item label="存储目录" prop="storePath">
              ...
              <el-select style="width: 100%" v-model="form.storePath" allow-create clearable filterable
                default-first-option :reserve-keyword="false" placeholder="请输入文件目录，默认yyyy/MMdd格式">
    ...
    <el-dialog title="查看" :lock-scroll="false" v-model="openView">
      <el-form ref="form" :model="formView" :rules="rules" label-width="auto">
        <el-row>
          <el-col :lg="12">
...
<script setup name="files">
  import { listSysfile, delSysfile, getSysfile } from '@/api/routine/files.js'
