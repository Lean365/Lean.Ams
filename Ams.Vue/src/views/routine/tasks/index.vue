    ...
    <el-form :model="queryParams" inline @submit.prevent ref="queryRef" v-show="searchToggle">
      ...
      <el-form-item prop="isStart">
        <el-radio-group v-model="queryParams.isStart" @change="handleQuery()">
          ...
          <el-radio-button :label="item.dictValue" v-for="item in options.isStartOptions">{{ item.dictLabel
            }}</el-radio-button>
        </el-radio-group>
      </el-form-item>
...
<script setup name="tasks">
  import { queryTasks, getTasks, createTasks, updateTasks, deleteTasks, startTasks, stopTasks, runTasks, exportTasks } from '@/api/routine/tasks'
  import { listJobLog } from '@/api/monitor/taskslog'
  ...
  function onJobLogView() {
    router.push({ path: '/routine/tasklog' })
  }
  /** 任务日志列表查询 */
