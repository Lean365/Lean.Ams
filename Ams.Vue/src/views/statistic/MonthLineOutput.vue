...
<template>
  <div>
    ...
    <el-table :data="dataList" border hight="200">
      ...
      <el-table-column prop="mfgLine" label="班组"></el-table-column>
      <el-table-column prop="mfgLot" label="批次"></el-table-column>
      <el-table-column prop="mfgModel" label="机种"></el-table-column>
      <el-table-column prop="mfgItem" label="物料"></el-table-column>
      <el-table-column prop="stdTime" label="工时"></el-table-column>
      <el-table-column prop="direct" label="直接"></el-table-column>
      <el-table-column prop="indirect" label="间接"></el-table-column>
    ...
    <!-- 添加或修改oph主表对话框 -->
    <div ref="chartRefLine" :class="className" :style="{height:height,width:width}" />
  </div>
</template>
...
<script setup name="monthoutput">
  ...
  import {
    getCountMonthLineOutput, getExportMonthLineOutput, getCountMonthLineOutputBar
  }
  ...
  let rows = [];
  let BarList = [];
  let stdOutputQty = reactive([]);
  let realOutputQty = reactive([]);
  let mfgLine = reactive([]);
  let achievedRate = reactive([]);
  let chart = null;
  const ids = ref([])
  ...
  //监听
  watch(() => [...achievedRate], () => {
    achievedRate.length = 0
  })
  watch(() => [...stdOutputQty], () => {
    stdOutputQty.length = 0
  })
  watch(() => [...realOutputQty], () => {
    realOutputQty.length = 0
  })
  watch(() => [...mfgLine], () => {
    mfgLine.length = 0
  })
  function getList() {
    ...
    loading.value = true
    getCountMonthLineOutput(queryParams).then(res => {
      const { code, data } = res
      if (code == 200) {
        console.log(data)
        dataList.value = data
    ...
    getCountMonthLineOutputBar(queryParams).then(res => {
      const { code, data } = res
      if (code == 200) {
        MapList = data;
        ...
        for (let i = 0; i < obj.length; i++) {
          achievedRate.push(MapList[i].achievedRate);
          stdOutputQty.push(MapList[i].stdOutputQty);
          realOutputQty.push(MapList[i].realOutputQty);
          mfgLine.push(MapList[i].mfgLine);
        }
        //动态插入数据时，数据在这里设置更新
        console.log("OBJ:", achievedRate);         // 确认数据有获取到
        chart.clear();
        chart.setOption(optionsMonthOutput, true)
      }
    })
  }
  ...
  function handleExport() {
    proxy
      ...
      .then(function () {
        return getExportMonthLineOutput(queryParams)
      })
      .then((response) => {
  ...
  const initChart = () => {
    ...
    chart = echarts.init(proxy.$refs.chartRefLine, 'macarons')
    //chart.setOption(optionsMonthOutput)
