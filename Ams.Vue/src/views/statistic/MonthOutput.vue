...
<template>
  <div>
    ...
    <!-- 添加或修改oph主表对话框 -->
    <div ref="chartRefMonth" :class="className" :style="{height:height,width:width}" />
  </div>
</template>
...
<script setup name="monthoutput">
    ...
    GetCountMonthOutput(queryParams).then(res => {
      ...
      if (code == 200) {
        ...
        for (let i = 0; i < obj.length; i++) {
          ...
          mfgDate.push(MapList[i].mfgDate);
        }
        //动态插入数据时，数据在这里设置更新
  ...
  const initChart = () => {
    ...
    chart = echarts.init(proxy.$refs.chartRefMonth, 'macarons')
