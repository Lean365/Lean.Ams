...
<script setup name="yearmodeloutput">
  import {
    getCountYearModelOutput, getExportYearModelOutput
  }
  ...
  function getList() {
    ...
    loading.value = true
    getCountYearModelOutput(queryParams).then(res => {
      const { code, data } = res
      if (code == 200) {
        console.log(data)
        dataList.value = data
  ...
  function handleExport() {
    proxy
      ...
      .then(function () {
        return getExportYearModelOutput(queryParams)
      })
      .then((response) => {
