...
<script setup name="chartHead">
  ...
  const carouselItems = ref([
    ...
    {
      id: 4,
      imageUrl: 'src/assets/logo/Lean.png'
    }
  ]);
  // // 初始化时间
  // const initTime = () => {
  ...
  nowYear.value = moment().year().toString().trim() //+ proxy.$t('common.Yearis') + moment().quarters() + proxy.$t('common.quarterth') + moment().weeksInYear() + proxy.$t('common.weekth') + moment().dayOfYear() + proxy.$t('common.Dayth')
