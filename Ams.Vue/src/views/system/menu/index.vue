<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      ...
      <el-form-item :label="$t('pmenu.menuState')" prop="isState">
        <el-select v-model="queryParams.isState" placeholder="菜单状态" clearable>
    ...
    <vxe-table :height="tableHeight" show-overflow ref="listRef" :loading="loading" :column-config="{ resizable: true }"
      ...
      <vxe-column field="visible" :title="$t('pmenu.isShow')" width="120" align="center">
        <template #default="scope">
          <dict-tag :options="sys_show_hide" :value="scope.row.visible" />
        </template>
      </vxe-column>
      <vxe-column field="isState" :title="$t('pmenu.menuState')" width="120" align="center">
        <template #default="scope">
          <dict-tag :options="sys_normal_disable" :value="scope.row.isState" />
        </template>
      </vxe-column>
      <vxe-column :title="$t('common.createTime')" align="center" field="createTime" show-overflow
          ...
          <el-col :lg="12" v-if="form.menuType != 'F'">
            <el-form-item>
              ...
              <el-radio-group v-model="form.isState">
                <el-radio v-for="dict in sys_normal_disable" :key="parseInt(dict.dictValue)"
