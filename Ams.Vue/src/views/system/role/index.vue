<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" v-show="showSearch" :inline="true">
      ...
      <el-form-item :label="$t('prole.isStatus')" prop="isState">
        <el-select v-model="queryParams.isState" :placeholder="$t('btn.select')+$t('prole.isStatus')" clearable>
    ...
    <el-table v-loading="loading" :data="roleList" height="650" highlight-current-row
      ...
      <el-table-column label="状态" width="90">
        <template #default="scope">
          <el-switch v-model="scope.row.isState" :disabled="scope.row.roleKey == 'admin'" :active-value="0"
        ...
        <template #default="scope">
          ...
          <div v-if="scope.row.roleKey != 'admin'">
            <el-button-group>
              ...
              <el-button size="small" type="danger" plain icon="delete" :title="$t('btn.delete')"
              ...
              <el-dropdown @command="(command) => handleCommand(command, scope.row)"
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item command="handleDataScope" icon="circle-check">{{ $t('prole.droleMenuPermi')
                      }}</el-dropdown-item>
                    <el-dropdown-item command="handleAuthUser" icon="user">{{ $t('prole.drolePermi')
                      }}</el-dropdown-item>
                    <el-dropdown-item command="handleExportMenu"
                      icon="download">{{$t('prole.exportMenu')}}</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </el-button-group>
            ...
            <template #default="{ node, data }">
              <div class="custom-tree-node">
                ...
                <span style="float;right: ;margin-left: 10px;">
                  <el-tag v-if="data.isState == 1" type="danger">停用</el-tag>
                </span>
              </div>
            </template>
          </el-tree>
...
<script setup name="role">
  ...
  function handleStatusChange(row) {
    const text = row.isState == '0' ? '启用' : '停用'
    ...
    proxy
      ...
      .then(function () {
        console.log(row.roleId, row.isState)
        return changeRoleStatus(row.roleId, row.isState)
      })
      .then(() => {
        proxy.$modal.msgSuccess(text + '成功')
      })
      .catch(function () {
        row.isState = row.isState == 0 ? 1 : 0
      })
  }
  // 取消按钮
  function cancel() {
