<template>
  <div class="component-upload-image">
    <el-upload multiple v-bind="$attrs" :action="uploadImgUrl" list-type="picture-card"
      :on-success="handleUploadSuccess" :before-upload="handleBeforeUpload" :limit="limit" :on-error="handleUploadError"
      :on-exceed="handleExceed" name="file" :data="uploadData" :on-remove="handleRemove" :show-file-list="true"
      :headers="headers" v-model:file-list="fileList" :on-preview="handlePictureCardPreview" :style="cssVars"
        ...
        <div class="el-upload__tip" v-if="showTip">
          ...
          <slot name="tip">
            ...
            <template v-if="fileType">
              {{$t('upload.Format')}} <b class="text-danger">{{ fileType.join('/') }}</b>
            </template>
            {{$t('upload.Filesfor')}}
          </slot>
        </div>
    ...
    <el-dialog v-model="dialogVisible" append-to-body>
      <el-form label-width="100px">
        <el-form-item :label="$t('btn.Preview')">
          <el-image style="display: block; max-width: 50%" :src="dialogImageUrl">
            <template #error>
              <div class="image-slot">{{$t('pcomponents.loadFailed')}}</div>
            </template>
          </el-image>
        </el-form-item>

        <el-form-item :label="$t('pcomponents.accessPath')">
          ...
          <el-button type="danger" text icon="document-copy" plain class="ml10" v-clipboard:success="copySuccess"
            v-clipboard:copy="dialogImageUrl">{{$t('btn.copy')}}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script setup>
  ...
  function handleUploadSuccess(res) {
    if (res.code != 200) {
      proxy.$modal.msgError(proxy.$t('upload.Reason') + `${res.msg}!`)
