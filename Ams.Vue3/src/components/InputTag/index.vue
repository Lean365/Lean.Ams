<template>
  <div class="flex gap-2">
    <el-tag v-for="tag in dynamicTags" :key="tag" class="mr10" closable :disable-transitions="false"
      @close="handleClose(tag)">
      {{ tag }}
    </el-tag>
    <el-input v-if="inputVisible" ref="InputRef" v-model="inputValue" :style="'width: ' + inputWidth" size="small"
      @keyup.enter="handleInputConfirm" @blur="handleInputConfirm" />
    <el-button v-if="!inputVisible" text icon="plus" size="small" @click="showInput">
      <slot name="btn"> {{$t('btn.add') }} </slot>
    </el-button>
  </div>
</template>
<script setup>
