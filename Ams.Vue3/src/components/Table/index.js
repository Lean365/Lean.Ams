...
export default {
  ...
  created() {
    ...
    if (cacheColumns && cacheColumns.length > 0) {
      ...
      let notExistsColums = this.columns.filter(m => !cacheColumns.find(c => c.dataIndex == m.dataIndex))
      cacheColumns.forEach((item,index) => {
        let temp = this.columns.find(m => m.dataIndex == item.dataIndex)
        if (temp) {
      ...
      let cacheLen = cacheColumns.length
      notExistsColums.forEach((item,index)=>{
        this.columns[cacheLen + index] = item
      })
      this.columnsSetting = this.columns
    }
    else {
