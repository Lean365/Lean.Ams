<template>
  <div id="tags-view-container" class="tags-view-container">
    <scroll-pane ref="scrollPaneRef" class="tags-view-wrapper" @scroll="handleScroll">
      <router-link v-for="tag in visitedViews" :key="tag.path" :data-path="tag.path" :class="{ active: isActive(tag) }"
    ...
    <ul v-show="visible" :style="{ left: left + 'px', top: top + 'px' }" class="contextmenu">
      <li @click="refreshSelectedTag(selectedTag)" v-if="isActive(selectedTag)">
        <refresh-right style="color:rgb(223, 172, 4); width: 1em; height: 1em" />
        {{ $t('tagsView.refresh') }}
      </li>
      <li v-if="!isAffix(selectedTag)" @click="closeSelectedTag(selectedTag)">
        <close style="color:rgb(175, 7, 7);width: 1em; height: 1em" /> {{ $t('tagsView.close') }}
      </li>
      <li @click="closeOthersTags"><circle-close style="color:rgb(42, 7, 197);width: 1em; height: 1em" /> {{
        $t('tagsView.closeOther') }}</li>
      <li v-if="!isFirstView()" @click="closeLeftTags">
        <back style="color:rgb(3, 78, 13);width: 1em; height: 1em" /> {{ $t('tagsView.closeLeft') }}
      </li>
      <li v-if="!isLastView()" @click="closeRightTags">
        <right style="color:rgb(5, 190, 15);width: 1em; height: 1em" /> {{ $t('tagsView.closeRight') }}
      </li>
      <li @click="closeAllTags(selectedTag)"><circle-close style="color:rgb(16, 1, 44);width: 1em; height: 1em" /> {{
        $t('tagsView.closeAll')
        }}</li>
    </ul>
  </div>
