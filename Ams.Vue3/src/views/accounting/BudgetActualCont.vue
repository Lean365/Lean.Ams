<!--
 * @Descripttion: 预算实际明细/fico_budget_actual_cont
 * @Version: 1.0.0.0
 * @Author: Lean365(Davis.Ching)
 * @Date: 2024/7/25 15:52:42
  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="统计类别" prop="fbExpCategory">
        <el-select filterable clearable   v-model="queryParams.fbExpCategory" :placeholder="$t('btn.selectSearchPrefix')+'统计类别'+$t('btn.selectSearchSuffix')">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/BudgetActualCont/importTemplate"
                  importUrl="/Accounting/BudgetActualCont/importData"
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fbFy" label="期间" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbFy')"/>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="SFID" prop="fbSfid">
              <el-input v-model.number="form.fbSfid" :placeholder="$t('btn.enterPrefix')+'SFID'+$t('btn.enterSuffix')" :disabled="opertype != 1"/>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="期间" prop="fbFy">
              <el-input v-model="form.fbFy" :placeholder="$t('btn.enterPrefix')+'期间'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="公司名称" prop="fbCorpName">
              <el-input v-model="form.fbCorpName" :placeholder="$t('btn.enterPrefix')+'公司名称'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="统计类别" prop="fbExpCategory">
              <el-select filterable clearable   v-model="form.fbExpCategory"  :placeholder="$t('btn.selectPrefix')+'统计类别'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="成本代码" prop="fbCostCode">
              <el-input v-model="form.fbCostCode" :placeholder="$t('btn.enterPrefix')+'成本代码'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="成本名称" prop="fbCostName">
              <el-input v-model="form.fbCostName" :placeholder="$t('btn.enterPrefix')+'成本名称'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="科目代码" prop="fbTitleCode">
              <el-input v-model="form.fbTitleCode" :placeholder="$t('btn.enterPrefix')+'科目代码'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="科目名称" prop="fbTitleName">
              <el-input v-model="form.fbTitleName" :placeholder="$t('btn.enterPrefix')+'科目名称'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="科目分类" prop="fbTitleNote">
              <el-input v-model="form.fbTitleNote" :placeholder="$t('btn.enterPrefix')+'科目分类'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="会计人员" prop="fbAccountant">
              <el-input v-model="form.fbAccountant" :placeholder="$t('btn.enterPrefix')+'会计人员'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
<script setup name="budgetactualcont">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listBudgetActualCont,
 addBudgetActualCont, delBudgetActualCont, 
 updateBudgetActualCont,getBudgetActualCont, 
 } 
from '@/api/accounting/budgetactualcont.js'
...
//API获取从预算实际明细/fico_budget_actual_cont表记录数据
function getList(){
  loading.value = true
  listBudgetActualCont(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleUpdate(row) {
  ...
  getBudgetActualCont(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.fbSfid != undefined && opertype.value === 2) {
        updateBudgetActualCont(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addBudgetActualCont(form.value).then((res) => {
...
function handleDelete(row) {
  ...
  proxy
    ...
    .then(function () {
      return delBudgetActualCont(Ids)
    })
    .then(() => {
      getList()
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/BudgetActualCont/export', { ...queryParams })
    })
}
