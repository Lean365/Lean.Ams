  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="名称" prop="fboClass">
        <el-input v-model="queryParams.fboClass" :placeholder="$t('btn.enterSearchPrefix')+'名称'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="启用标记" prop="fboFlag">
        <el-select filterable clearable   v-model="queryParams.fboFlag" :placeholder="$t('btn.selectSearchPrefix')+'启用标记'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="审核" prop="fboAudit">
        <el-select filterable clearable   v-model="queryParams.fboAudit" :placeholder="$t('btn.selectSearchPrefix')+'审核'+$t('btn.selectSearchSuffix')">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/BudgetOvertime/importTemplate"
                  importUrl="/Accounting/BudgetOvertime/importData"
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fboFlag" label="启用标记" align="center" v-if="columns.showColumn('fboFlag')">
        <template #default="scope">
          <dict-tag :options=" options.sys_flag_list " :value="scope.row.fboFlag"  />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="fboAuditName" label="审核人员" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fboAuditName')"/>
      <el-table-column prop="fboAuditDate" label="审核日期" :show-overflow-tooltip="true"  v-if="columns.showColumn('fboAuditDate')"/>
      <el-table-column prop="fboUndoName" label="撤消人员" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fboUndoName')"/>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="加班事由" prop="fboContent">
              <el-input v-model="form.fboContent" :placeholder="$t('btn.enterPrefix')+'加班事由'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="审核人员" prop="fboAuditName">
              <el-input v-model="form.fboAuditName" :placeholder="$t('btn.enterPrefix')+'审核人员'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="撤消人员" prop="fboUndoName">
              <el-input v-model="form.fboUndoName" :placeholder="$t('btn.enterPrefix')+'撤消人员'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
<script setup name="budgetovertime">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listBudgetOvertime,
 addBudgetOvertime, delBudgetOvertime, 
 updateBudgetOvertime,getBudgetOvertime, 
 } 
from '@/api/accounting/budgetovertime.js'
...
function getList(){
  ...
  loading.value = true
  listBudgetOvertime(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.fboSfid != undefined && opertype.value === 2) {
        updateBudgetOvertime(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addBudgetOvertime(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/BudgetOvertime/export', { ...queryParams })
    })
}
