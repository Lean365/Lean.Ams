  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="名称" prop="fbsClass">
        <el-input v-model="queryParams.fbsClass" :placeholder="$t('btn.enterSearchPrefix')+'名称'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/BudgetStaff/importTemplate"
                  importUrl="/Accounting/BudgetStaff/importData"
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fbsCategory" label="类别" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbsCategory')"/>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="类别" prop="fbsCategory">
              <el-input v-model="form.fbsCategory" :placeholder="$t('btn.enterPrefix')+'类别'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
<script setup name="budgetstaff">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listBudgetStaff,
 addBudgetStaff, delBudgetStaff, 
 updateBudgetStaff,getBudgetStaff, 
 } 
from '@/api/accounting/budgetstaff.js'
...
function getList(){
  ...
  loading.value = true
  listBudgetStaff(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
const state = reactive({
  ...
  rules: {
    ...
    fbsCategory: [{ required: true, message: "类别"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.fbsSFID != undefined && opertype.value === 2) {
        updateBudgetStaff(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addBudgetStaff(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/BudgetStaff/export', { ...queryParams })
    })
}
