<!--
 * @Descripttion: bom成本/fico_costing_bom
 * @Version: 1.0.0.0
 * @Author: Lean365(Davis.Ching)
 * @Date: 2024/7/25 15:51:23
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['fico:costingbom:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['fico:costingbom:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/CostingBom/importTemplate"
                  importUrl="/Accounting/CostingBom/importData"
...
<script setup name="costingbom">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listCostingBom,
 addCostingBom, delCostingBom, 
 updateCostingBom,getCostingBom, 
 } 
from '@/api/accounting/costingbom.js'
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'bom成本'
  opertype.value = 1
  form.value.bcPlant= []
  form.value.bcYm= []
...
function handleUpdate(row) {
  ...
  getCostingBom(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ 'bom成本'
      opertype.value = 2
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.bcSfid != undefined && opertype.value === 2) {
        updateCostingBom(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addCostingBom(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<bom成本.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Accounting/CostingBom/export', { ...queryParams })
    })
}
