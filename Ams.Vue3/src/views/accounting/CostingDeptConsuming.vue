          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/CostingDeptConsuming/importTemplate"
                  importUrl="/Accounting/CostingDeptConsuming/importData"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="期间" prop="dcFy">
              <el-input v-model="form.dcFy" :placeholder="$t('btn.enterPrefix')+'期间'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="工厂" prop="dcPlant">
              <el-input v-model="form.dcPlant" :placeholder="$t('btn.enterPrefix')+'工厂'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
<script setup name="costingdeptconsuming">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listCostingDeptConsuming,
 addCostingDeptConsuming, delCostingDeptConsuming, 
 updateCostingDeptConsuming,getCostingDeptConsuming, 
 } 
from '@/api/accounting/costingdeptconsuming.js'
...
function getList(){
  loading.value = true
  listCostingDeptConsuming(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
const state = reactive({
  ...
  rules: {
    ...
    dcPlant: [{ required: true, message: "工厂"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
...
function handleUpdate(row) {
  ...
  getCostingDeptConsuming(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.dcSfid != undefined && opertype.value === 2) {
        updateCostingDeptConsuming(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addCostingDeptConsuming(form.value).then((res) => {
...
function handleDelete(row) {
  ...
  proxy
    ...
    .then(function () {
      return delCostingDeptConsuming(Ids)
    })
    .then(() => {
      getList()
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/CostingDeptConsuming/export', { ...queryParams })
    })
}
