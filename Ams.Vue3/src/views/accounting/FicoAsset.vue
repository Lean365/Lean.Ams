  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="单位" prop="faAssetUnit">
        <el-select filterable clearable   v-model="queryParams.faAssetUnit" :placeholder="$t('btn.selectSearchPrefix')+'单位'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="管理区分" prop="faAssetMana">
        <el-radio-group v-model="queryParams.faAssetMana">
          <el-radio>{{$t('common.all')}}</el-radio>
          <el-radio v-for="item in  options.sys_assets_dist " :key="item.dictValue" :value="item.dictValue">{{item.dictLabel}}</el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="存放位置" prop="faLocale">
        <el-input v-model="queryParams.faLocale" :placeholder="$t('btn.enterSearchPrefix')+'存放位置'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="使用年限" prop="faAssetUsed">
        <el-input v-model="queryParams.faAssetUsed" :placeholder="$t('btn.enterSearchPrefix')+'使用年限'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="是否停用" prop="faAssetStop">
        <el-radio-group v-model="queryParams.faAssetStop">
          <el-radio>{{$t('common.all')}}</el-radio>
      ...
      <el-form-item label="资产形态" prop="faEntity">
        <el-radio-group v-model="queryParams.faEntity">
          <el-radio>{{$t('common.all')}}</el-radio>
          <el-radio v-for="item in  options.sys_assets_pattern " :key="item.dictValue" :value="item.dictValue">{{item.dictLabel}}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="资产存在否" prop="faHaveorNot">
        <el-radio-group v-model="queryParams.faHaveorNot">
          <el-radio>{{$t('common.all')}}</el-radio>
          <el-radio v-for="item in  options.sys_is_status " :key="item.dictValue" :value="item.dictValue">{{item.dictLabel}}</el-radio>
        </el-radio-group>
      </el-form-item>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['fico:asset:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['fico:asset:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['fico:asset:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/FicoAsset/importTemplate"
                  importUrl="/Accounting/FicoAsset/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['fico:asset:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['fico:asset:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['fico:asset:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="管理区分" prop="faAssetMana">
              <el-radio-group v-model="form.faAssetMana">
                <el-radio v-for="item in options.sys_assets_dist" :key="item.dictValue" :value="parseInt(item.dictValue)">
                  {{item.dictLabel}}
                </el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="资产存在否" prop="faHaveorNot">
              <el-radio-group v-model="form.faHaveorNot">
                <el-radio v-for="item in options.sys_is_status" :key="item.dictValue" :value="parseInt(item.dictValue)">
                  {{item.dictLabel}}
                </el-radio>
              </el-radio-group>
...
const state = reactive({
  ...
  rules: {
    ...
    faCorp: [{ required: true, message: "公司"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faDept: [{ required: true, message: "使用部门"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faClassCode: [{ required: true, message: "资产类别"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faAssetNo: [{ required: true, message: "资产编码"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faCostCenter: [{ required: true, message: "成本中心"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faAssetShortName: [{ required: true, message: "资产简称"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faAssetFullName: [{ required: true, message: "资产全称"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faAssetUnit: [{ required: true, message: "单位"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faAssetQty: [{ required: true, message: "数量"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    faAssetMana: [{ required: true, message: "管理区分"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    faAssetCcy: [{ required: true, message: "币种"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    faAcquisitionPrice: [{ required: true, message: "购买价格"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faCapitalizedDate: [{ required: true, message: "资本化日期"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faDepreciationValue: [{ required: true, message: "折旧价值"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faLocale: [{ required: true, message: "存放位置"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faAssetUsed: [{ required: true, message: "使用年限"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faAssetStop: [{ required: true, message: "是否停用"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    faEntity: [{ required: true, message: "资产形态"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    faHaveorNot: [{ required: true, message: "资产存在否"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    faCheckedby: [{ required: true, message: "盘点人员"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    faCheckedDate: [{ required: true, message: "盘点日期"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/FicoAsset/export', { ...queryParams })
    })
}
