  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="核算日期">
        <el-date-picker
          v-model="dateRangeBcBalancedate" 
          type="datetimerange"
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['fico:bomcosting:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['fico:bomcosting:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['fico:bomcosting:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/FicoBomCosting/importTemplate"
                  importUrl="/Accounting/FicoBomCosting/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['fico:bomcosting:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="bcFy" label="期间" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('bcFy')"/>
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['fico:bomcosting:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['fico:bomcosting:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="期间" prop="bcFy">
              <el-input v-model="form.bcFy" :placeholder="$t('btn.enterPrefix')+'期间'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
//后台操作函数
import { listFicoBomCosting,
 addFicoBomCosting, delFicoBomCosting, 
 updateFicoBomCosting,getFicoBomCosting, 
 } 
from '@/api/accounting/ficobomcosting.js'
...
// 核算日期时间范围
const dateRangeBcBalancedate = ref([])
...
function getList(){
  proxy.addDateRange(queryParams, dateRangeBcBalancedate.value, 'BcBalancedate');
  loading.value = true
...
const state = reactive({
  ...
  rules: {
    ...
    bcFy: [{ required: true, message: "期间"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<bom成本核算.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Accounting/FicoBomCosting/export', { ...queryParams })
    })
}
