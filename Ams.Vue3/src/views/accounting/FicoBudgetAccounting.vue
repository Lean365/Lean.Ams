...
<template>
  <div>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :lg="24">
          ...
          <el-form-item label="会计科目" prop="md008">
            <el-select-v2 filterable clearable v-model="queryParams.md008" :options="filteredParamsSaknr"
              :placeholder="$t('btn.selectSearchPrefix')+'会计科目'+$t('btn.selectSearchSuffix')">
            </el-select-v2>
          </el-form-item>
          <el-form-item label="名称" prop="md009">
            <el-input v-model="queryParams.md009"
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['fico:budgetaccounting:delete']" plain
          icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['fico:budgetaccounting:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData templateUrl="Accounting/FicoBudgetAccounting/importTemplate"
                  importUrl="/Accounting/FicoBudgetAccounting/importData" @success="handleFileSuccess"></importData>
              </el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="download" @click="handleExport"
          v-hasPermi="['fico:budgetaccounting:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <!-- 数据区域 -->
    <el-table v-if="refreshTable" :data="dataList" v-loading="loading" ref="tableRef" border height="600px"
      highlight-current-row @selection-change="handleSelectionChange" :default-expand-all="isExpandAll" row-key="id"
      :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
      <el-table-column type="selection" width="50" align="center" />
      <el-table-column prop="id" label="ID" align="center" v-if="columns.showColumn('id')" />
      <el-table-column prop="parentId" label="父ID" align="center" v-if="columns.showColumn('parentId')" />
      <el-table-column prop="md003" label="集团" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('md003')" />
      ...
      <el-table-column prop="md005" label="语言Key" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('md005')" />
      <el-table-column prop="md006" label="预算科目" align="center" :show-overflow-tooltip="true"
      ...
      <el-table-column prop="md008" label="会计科目" align="center" v-if="columns.showColumn('md008')">
        <!-- <template #default="scope">
          <dict-tag :options=" options.sql_accounting_title " :value="scope.row.md008" />
        </template> -->
      </el-table-column>
      <el-table-column prop="md009" label="名称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('md009')" />
      <el-table-column prop="md010" label="长文本" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('md010')" />
      <el-table-column prop="md011" label="附加文本" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('md011')" />
      <el-table-column prop="md012" label="费用类型" align="center" v-if="columns.showColumn('md012')">
        <template #default="scope">
          <dict-tag :options=" options.sys_costs_type " :value="scope.row.md012" />
        </template>
      </el-table-column>
      <el-table-column prop="md013" label="统驭类别" align="center" v-if="columns.showColumn('md013')">
        <template #default="scope">
          <dict-tag :options=" options.sys_conrol_title " :value="scope.row.md013" />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operation')" align="center" width="140">
        <template #default="scope">
          <el-button v-hasPermi="['fico:budgetaccounting:edit']" class="btn-edit" icon="edit" size="small"
            :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['fico:budgetaccounting:delete']" class="btn-delete" icon="delete" size="small"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="24">
            <el-form-item :label="$t('common.tipParentId')" prop="parentId">
              <el-cascader class="w100" :options="dataList"
                :props="{ checkStrictly: true, value: 'id', label: 'md009', emitPath: false }"
                :placeholder="$t('common.tipPreviousName')" clearable v-model="form.parentId">
                <template #default="{ node, data }">
                  <span>{{ data.md009 }}</span>
          ...
          <el-col :lg="12">
            <el-form-item label="集团" prop="md003">
              <el-input v-model="form.md003" :placeholder="$t('btn.enterPrefix')+'集团'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="公司代码" prop="md004">
              <el-select filterable clearable v-model="form.md004"
                :placeholder="$t('btn.selectPrefix')+'公司代码'+$t('btn.selectSuffix')" @change="handleBukrsChange">
                <el-option v-for="item in  options.sql_corp_list" :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :lg="12">
            <el-form-item label="语言Key" prop="md005">
              <el-input v-model="form.md005" :placeholder="$t('btn.enterPrefix')+'语言Key'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="预算科目" prop="md006">
              <el-input v-model="form.md006" :placeholder="$t('btn.enterPrefix')+'预算科目'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="币种" prop="md007">
              <el-select filterable clearable v-model="form.md007"
          ...
          <el-col :lg="12">
            <el-form-item label="会计科目" prop="md008">
              <el-select-v2 filterable clearable v-model="form.md008"
                :placeholder="$t('btn.selectPrefix')+'会计科目'+$t('btn.selectSuffix')" :options="filteredFormSaknr">
              </el-select-v2>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="名称" prop="md009">
              <el-input v-model="form.md009" :placeholder="$t('btn.enterPrefix')+'名称'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="长文本" prop="md010">
              <el-input v-model="form.md010" :placeholder="$t('btn.enterPrefix')+'长文本'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="附加文本" prop="md011">
              <el-input v-model="form.md011" :placeholder="$t('btn.enterPrefix')+'附加文本'+$t('btn.enterSuffix')"
          ...
          <el-col :lg="12">
            <el-form-item label="统驭类别" prop="md013">
              <el-select filterable clearable v-model="form.md013"
...
<script setup name="ficobudgetaccounting">
  ...
  import {
    treelistFicoBudgetAccounting, listFicoBudgetAccounting,
  ...
  const columns = ref([
    ...
    { visible: false, prop: 'md003', label: '集团' },
    { visible: false, prop: 'md004', label: '公司代码' },
    { visible: false, prop: 'md005', label: '语言Key' },
    { visible: true, prop: 'md006', label: '预算科目' },
    { visible: false, prop: 'md007', label: '币种' },
    { visible: true, prop: 'md008', label: '会计科目' },
    { visible: true, prop: 'md009', label: '名称' },
    { visible: true, prop: 'md010', label: '长文本' },
    { visible: true, prop: 'md011', label: '附加文本' },
    { visible: false, prop: 'md012', label: '费用类型' },
    { visible: false, prop: 'md013', label: '统驭类别' },
    { visible: true, prop: 'md014', label: '冻结' },
  ...
  const filteredParamsSaknr = ref([])
  const filteredFormSaknr = ref([])
  function handleBukrsChange() {
    if (open.value === true) {

      form.value.md007 = ''
      form.value.md008 = ''
      filteredFormSaknr.value = state.options.sql_accounting_title.filter(item => item.extLabel === form.value.md004).map(item => ({ label: item.dictLabel, value: item.dictValue }))
      //console.log(filteredFormSaknr)
      if (form.value.md004 === '2300') {
        form.value.md007 = 'CNY'
      }
      if (form.value.md004 === '2400') {
        form.value.md007 = 'HKD'
      }
      if (form.value.md004 === '2700') {
        form.value.md007 = 'CNY'
      }
      if (form.value.md004 === '1000') {
        form.value.md007 = 'JPY'
      }
      if (form.value.md004 === '3000') {
        form.value.md007 = 'USA'
      }
      if (form.value.md004 === '4000') {
        form.value.md007 = 'EUR'
      }
      if (form.value.md004 === '4100') {
        form.value.md007 = 'EUR'
      }
    } else {
      form.value.md007 = ''
      form.value.md008 = ''
      //console.log('state.options.sql_accounting_title')
      filteredParamsSaknr.value = state.options.sql_accounting_title.filter(item => item.extLabel === queryParams.md004).map(item => ({ label: item.dictLabel, value: item.dictValue }))
      //console.log('handleBukrsChange')

      if (queryParams.md004 === '2300') {
        queryParams.md007 = 'CNY'
      }
      if (queryParams.md004 === '2400') {
        queryParams.md007 = 'HKD'
      }
      if (queryParams.md004 === '2700') {
        queryParams.md007 = 'CNY'
      }
      if (queryParams.md004 === '1000') {
        queryParams.md007 = 'JPY'
      }
      if (queryParams.md004 === '3000') {
        queryParams.md007 = 'USD'
      }
      if (queryParams.md004 === '4000') {
        queryParams.md007 = 'EUR'
      }
      if (queryParams.md004 === '4100') {
        queryParams.md007 = 'EUR'
      }
    }
  }
