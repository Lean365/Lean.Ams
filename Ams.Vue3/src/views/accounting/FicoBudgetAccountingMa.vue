  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="公司代码" prop="fbasBukrs">
        <el-select filterable clearable   v-model="queryParams.fbasBukrs" :placeholder="$t('btn.selectSearchPrefix')+'公司代码'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="预算科目" prop="fbasFipls">
        <el-input v-model="queryParams.fbasFipls" :placeholder="$t('btn.enterSearchPrefix')+'预算科目'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="会计科目" prop="fbasSaknr">
        <el-select filterable clearable   v-model="queryParams.fbasSaknr" :placeholder="$t('btn.selectSearchPrefix')+'会计科目'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="名称" prop="fbasSatext">
        <el-input v-model="queryParams.fbasSatext" :placeholder="$t('btn.enterSearchPrefix')+'名称'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="长文本" prop="fbasLtext">
        <el-input v-model="queryParams.fbasLtext" :placeholder="$t('btn.enterSearchPrefix')+'长文本'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="费用类型" prop="fbasGvtyp">
        <el-select filterable clearable   v-model="queryParams.fbasGvtyp" :placeholder="$t('btn.selectSearchPrefix')+'费用类型'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="冻结" prop="fbasXspea">
        <el-select filterable clearable   v-model="queryParams.fbasXspea" :placeholder="$t('btn.selectSearchPrefix')+'冻结'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="统驭科目" prop="fbasMitkz">
        <el-select filterable clearable   v-model="queryParams.fbasMitkz" :placeholder="$t('btn.selectSearchPrefix')+'统驭科目'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="币种" prop="fbasWaers">
        <el-select filterable clearable   v-model="queryParams.fbasWaers" :placeholder="$t('btn.selectSearchPrefix')+'币种'+$t('btn.selectSearchSuffix')">
    ...
    <el-row :gutter="15" class="mb10">
      ...
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['fico:budgetaccountingma:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['fico:budgetaccountingma:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/FicoBudgetAccountingMa/importTemplate"
                  importUrl="/Accounting/FicoBudgetAccountingMa/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['fico:budgetaccountingma:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fbasMandt" label="集团" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasMandt')"/>
      ...
      <el-table-column prop="fbasSpras" label="语言Key" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasSpras')"/>
      <el-table-column prop="fbasFipls" label="预算科目" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasFipls')"/>
      ...
      <el-table-column prop="fbasSatext" label="名称" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasSatext')"/>
      <el-table-column prop="fbasLtext" label="长文本" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasLtext')"/>
      <el-table-column prop="fbasStext" label="附加文本" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fbasStext')"/>
      ...
      <el-table-column prop="fbasMitkz" label="统驭科目" align="center" v-if="columns.showColumn('fbasMitkz')">
        <template #default="scope">
          <dict-tag :options=" options.sys_normal_whether " :value="scope.row.fbasMitkz"  />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['fico:budgetaccountingma:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['fico:budgetaccountingma:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ficoBudgetAccountingSlvList" header-row-class-name="text-navy">
        ...
        <el-table-column prop="fbasSatext" label="名称"/>
        <el-table-column prop="fbasLtext" label="长文本"/>
        <el-table-column prop="fbasStext" label="附加文本"/>
        <el-table-column prop="fbasGvtyp" label="费用类型"/>
        <el-table-column prop="fbasXspea" label="冻结">
          <template #default="scope">
            <dict-tag :options=" options.sys_freeze_flag " :value="scope.row.fbasXspea"  />
          </template>
        </el-table-column>
        <el-table-column prop="fbasMitkz" label="统驭科目">
          <template #default="scope">
            <dict-tag :options=" options.sys_normal_whether " :value="scope.row.fbasMitkz"  />
          </template>
        </el-table-column>
        <el-table-column prop="fbasWaers" label="币种">
          <template #default="scope">
            <dict-tag :options=" options.sys_ccy_type " :value="scope.row.fbasWaers"  />
          </template>
        </el-table-column>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="ID" prop="fbasSfid">
              <el-input v-model.number="form.fbasSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" :disabled="opertype != 1"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="公司代码" prop="fbasBukrs">
              <el-select filterable clearable   v-model="form.fbasBukrs"  :placeholder="$t('btn.selectPrefix')+'公司代码'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="语言Key" prop="fbasSpras">
              <el-input v-model="form.fbasSpras" :placeholder="$t('btn.enterPrefix')+'语言Key'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="会计科目" prop="fbasSaknr">
              <el-select filterable clearable   v-model="form.fbasSaknr"  :placeholder="$t('btn.selectPrefix')+'会计科目'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="名称" prop="fbasSatext">
              <el-input v-model="form.fbasSatext" :placeholder="$t('btn.enterPrefix')+'名称'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="附加文本" prop="fbasStext">
              <el-input v-model="form.fbasStext" :placeholder="$t('btn.enterPrefix')+'附加文本'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="费用类型" prop="fbasGvtyp">
              <el-select filterable clearable   v-model="form.fbasGvtyp"  :placeholder="$t('btn.selectPrefix')+'费用类型'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="冻结" prop="fbasXspea">
              <el-select filterable clearable   v-model="form.fbasXspea"  :placeholder="$t('btn.selectPrefix')+'冻结'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="统驭科目" prop="fbasMitkz">
              <el-select filterable clearable   v-model="form.fbasMitkz"  :placeholder="$t('btn.selectPrefix')+'统驭科目'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="币种" prop="fbasWaers">
              <el-select filterable clearable   v-model="form.fbasWaers"  :placeholder="$t('btn.selectPrefix')+'币种'+$t('btn.selectSuffix')">
                <el-option
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">预算科目</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddFicoBudgetAccountingSlv">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeleteFicoBudgetAccountingSlv">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ficoBudgetAccountingSlvList" :row-class-name="rowFicoBudgetAccountingSlvIndex" @selection-change="handleFicoBudgetAccountingSlvSelectionChange" ref="FicoBudgetAccountingSlvRef">
          ...
          <el-table-column label="语言Key" align="center" prop="fbasSpras">
            <template #default="scope">
              <el-input v-model="scope.row.fbasSpras" :placeholder="$t('btn.enterPrefix')+'语言Key'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="明细科目" align="center" prop="fbasFdpls">
            <template #default="scope">
              <el-input v-model="scope.row.fbasFdpls" :placeholder="$t('btn.enterPrefix')+'明细科目'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="名称" align="center" prop="fbasSatext">
            <template #default="scope">
              <el-input v-model="scope.row.fbasSatext" :placeholder="$t('btn.enterPrefix')+'名称'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="长文本" align="center" prop="fbasLtext">
            <template #default="scope">
              <el-input v-model="scope.row.fbasLtext" :placeholder="$t('btn.enterPrefix')+'长文本'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="附加文本" align="center" prop="fbasStext">
            <template #default="scope">
              <el-input v-model="scope.row.fbasStext" :placeholder="$t('btn.enterPrefix')+'附加文本'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="费用类型" align="center" prop="fbasGvtyp">
            <template #default="scope">
              <el-input v-model="scope.row.fbasGvtyp" :placeholder="$t('btn.enterPrefix')+'费用类型'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="冻结" prop="fbasXspea">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.fbasXspea" :placeholder="$t('btn.enterPrefix')+'冻结'+$t('btn.enterSuffix')">
                <el-option
                  v-for="item in options.sys_freeze_flag" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="统驭科目" prop="fbasMitkz">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.fbasMitkz" :placeholder="$t('btn.enterPrefix')+'统驭科目'+$t('btn.enterSuffix')">
                <el-option
                  v-for="item in options.sys_normal_whether" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="币种" prop="fbasWaers">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.fbasWaers" :placeholder="$t('btn.enterPrefix')+'币种'+$t('btn.enterSuffix')">
                <el-option
                  v-for="item in options.sys_ccy_type" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
...
//后台操作函数
import { listFicoBudgetAccountingMa,
...
const columns = ref([
  ...
  { visible: true, prop: 'fbasSatext', label: '名称' },
  { visible: true, prop: 'fbasLtext', label: '长文本' },
  { visible: false, prop: 'fbasStext', label: '附加文本' },
  { visible: false, prop: 'fbasGvtyp', label: '费用类型' },
  { visible: false, prop: 'fbasXspea', label: '冻结' },
  { visible: false, prop: 'fbasMitkz', label: '统驭科目' },
  { visible: false, prop: 'fbasWaers', label: '币种' },
...
const state = reactive({
  ...
  rules: {
    ...
    fbasMandt: [{ required: true, message: "集团"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    fbasBukrs: [{ required: true, message: "公司代码"+proxy.$t('btn.isEmpty'), trigger: "change"     }],
    fbasSpras: [{ required: true, message: "语言Key"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
  },
  options: {
    // 公司代码 选项列表 格式 eg:{ dictLabel: '标签', dictValue: '0'}
sys_crop_list: [],
    // 会计科目 选项列表 格式 eg:{ dictLabel: '标签', dictValue: '0'}
...
function handleDeleteFicoBudgetAccountingSlv() {
  if(checkedFicoBudgetAccountingSlv.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的预算科目数据')
  } else {
