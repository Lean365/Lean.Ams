...
<template>
  <div>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :lg="24">
          ...
          <el-form-item label="会计科目" prop="fbasSaknr">
            <el-select filterable clearable v-model="queryParams.fbasSaknr"
              :placeholder="$t('btn.selectSearchPrefix')+'会计科目'+$t('btn.selectSearchSuffix')">
              <el-option v-for="item in   options.sql_accounting_title " :key="item.dictValue" :label="item.dictLabel"
                :value="item.dictValue">
                <span class="fl">{{ item.dictLabel }}</span>
                <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>
              </el-option>
          ...
          <el-form-item label="名称" prop="fbasSatext">
            <el-input v-model="queryParams.fbasSatext"
          ...
          <el-form-item label="冻结" prop="fbasXspea">
            <el-select filterable clearable v-model="queryParams.fbasXspea"
              :placeholder="$t('btn.selectSearchPrefix')+'冻结'+$t('btn.selectSearchSuffix')">
    ...
    <el-table border height="600px" :data="dataList" v-loading="loading" ref="table"
      ...
      <el-table-column prop="fbasSatext" label="名称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fbasSatext')" />
      <el-table-column prop="fbasLtext" label="长文本" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fbasLtext')" />
      <el-table-column prop="fbasStext" label="附加文本" align="center" :show-overflow-tooltip="true"
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
          <el-tab-pane :label="$t('ptabs.basicInfo')" name="first">
            <el-row :gutter="20">
              ...
              <el-col :lg="12">
                <el-form-item label="预算科目" prop="fbasFipls">
                  <el-input v-model="form.fbasFipls"
                    :placeholder="$t('btn.enterPrefix')+'预算科目'+$t('btn.enterSuffix')" />
                </el-form-item>
              </el-col>

              <el-col :lg="12">
                <el-form-item label="会计科目" prop="fbasSaknr">
                  <el-select filterable clearable v-model="form.fbasSaknr"
                    :placeholder="$t('btn.selectPrefix')+'会计科目'+$t('btn.selectSuffix')">
                    <el-option v-for="item in  options.sql_accounting_title" :key="item.dictValue"
              ...
              <el-col :lg="12">
                <el-form-item label="附加文本" prop="fbasStext">
                  <el-input v-model="form.fbasStext"
              ...
              <el-col :lg="12">
                <el-form-item label="冻结" prop="fbasXspea">
                  <el-select filterable clearable v-model="form.fbasXspea"
                    :placeholder="$t('btn.selectPrefix')+'冻结'+$t('btn.selectSuffix')">
                    <el-option v-for="item in  options.sys_freeze_flag" :key="item.dictValue" :label="item.dictLabel"
        ...
        <el-table :data="ficoBudgetAccountingSlvList" :row-class-name="rowFicoBudgetAccountingSlvIndex"
          ...
          <el-table-column label="名称" align="center" prop="fbasSatext">
            <template #default="scope">
              <el-input v-model="scope.row.fbasSatext"
          ...
          <el-table-column label="费用类型" align="center" prop="fbasGvtyp">
            <template #default="scope">
              <el-input v-model="scope.row.fbasGvtyp"
                :placeholder="$t('btn.enterPrefix')+'费用类型'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="统驭科目" prop="fbasMitkz">
            <template #default="scope">
              <el-select filterable clearable v-model="scope.row.fbasMitkz"
                :placeholder="$t('btn.enterPrefix')+'统驭科目'+$t('btn.enterSuffix')">
                <el-option v-for="item in options.sys_normal_whether" :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </template>
          ...
          <el-table-column label="币种" prop="fbasWaers">
            <template #default="scope">
              <el-select filterable clearable v-model="scope.row.fbasWaers"
                :placeholder="$t('btn.enterPrefix')+'币种'+$t('btn.enterSuffix')">
                <el-option v-for="item in options.sys_ccy_type" :key="item.dictValue" :label="item.dictLabel"
                  :value="parseInt(item.dictValue)"></el-option>
              </el-select>
            </template>
