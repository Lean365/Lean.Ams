<!--
 * @Descripttion: 明细科目/fico_budget_accounting_slv
 * @Version: 1.0.0.0
...
<template>
  <div>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :lg="24">
          <el-form-item label="明细科目  " prop="fbasFdpls">
            <el-input v-model="queryParams.fbasFdpls"
              :placeholder="$t('btn.enterSearchPrefix')+'明细科目  '+$t('btn.enterSearchSuffix')" />
          </el-form-item>
    ...
    <el-row :gutter="15" class="mb10">
      ...
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['fico:budgetaccountingslv:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['fico:budgetaccountingslv:delete']" plain
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData templateUrl="Accounting/FicoBudgetAccountingSlv/importTemplate"
                  importUrl="/Accounting/FicoBudgetAccountingSlv/importData" @success="handleFileSuccess"></importData>
              </el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport"
          v-hasPermi="['fico:budgetaccountingslv:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px" :data="dataList" v-loading="loading" ref="table"
      ...
      <el-table-column prop="fbasFdpls" label="明细科目  " align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fbasFdpls')" />
        ...
        <template #default="scope">
          <el-button-group>
            <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')"
              v-hasPermi="['fico:budgetaccountingslv:edit']" @click="handleUpdate(scope.row)"></el-button>
            <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')"
              v-hasPermi="['fico:budgetaccountingslv:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
              ...
              <el-col :lg="12">
                <el-form-item label="语言Key  " prop="fbasSpras">
                  <el-input v-model="form.fbasSpras"
                    :placeholder="$t('btn.enterPrefix')+'语言Key  '+$t('btn.enterSuffix')" show-word-limit
                    maxlength="255" />
                </el-form-item>
              </el-col>
...
<script setup name="ficobudgetaccountingslv">
  ...
  //API获取从明细科目/fico_budget_accounting_slv表记录数据
  function getList() {
    loading.value = true
  ...
  function handleAdd() {
    reset();
    open.value = true
    title.value = proxy.$t('btn.add') + " " + '明细科目'
    opertype.value = 1
  ...
  function handleUpdate(row) {
    ...
    getFicoBudgetAccountingSlv(id).then((res) => {
      ...
      if (code == 200) {
        open.value = true
        title.value = proxy.$t('btn.edit') + " " + '明细科目'
        opertype.value = 2
  ...
  function handleExport() {
    proxy
      .$confirm(proxy.$t('common.tipConfirmExport') + "<明细科目.xlsx>", proxy.$t('btn.export') + ' ' + proxy.$t('common.tip'), {
        confirmButtonText: proxy.$t('btn.submit'),
        cancelButtonText: proxy.$t('btn.cancel'),
        type: "warning",
      })
