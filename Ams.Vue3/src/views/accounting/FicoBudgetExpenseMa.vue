  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      <el-row :gutter="10" class="mb8">
        <el-col :lg="24">
      <el-form-item label="期间" prop="fbeLfgja">
        <el-select filterable clearable   v-model="queryParams.fbeLfgja" :placeholder="$t('btn.selectSearchPrefix')+'期间'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="公司" prop="fbeBukrs">
        <el-select filterable clearable   v-model="queryParams.fbeBukrs" :placeholder="$t('btn.selectSearchPrefix')+'公司'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="部门" prop="fbeAbtnr">
        <el-select filterable clearable   v-model="queryParams.fbeAbtnr" :placeholder="$t('btn.selectSearchPrefix')+'部门'+$t('btn.selectSearchSuffix')">
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fbeLfgja" label="期间" align="center" v-if="columns.showColumn('fbeLfgja')">
        <template #default="scope">
          <dict-tag :options=" options.sql_attr_list " :value="scope.row.fbeLfgja"  />
        </template>
      </el-table-column>
      <el-table-column prop="fbeBukrs" label="公司" align="center" v-if="columns.showColumn('fbeBukrs')">
        <template #default="scope">
          <dict-tag :options=" options.sql_corp_list " :value="scope.row.fbeBukrs"  />
        </template>
      </el-table-column>
      <el-table-column prop="fbeAbtnr" label="部门" align="center" v-if="columns.showColumn('fbeAbtnr')">
        <template #default="scope">
          <dict-tag :options=" options.sql_dept_name " :value="scope.row.fbeAbtnr"  />
        </template>
      </el-table-column>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ficoBudgetExpenseSlvList" header-row-class-name="text-navy">
        ...
        <el-table-column prop="fbesLfmon" label="年月"/>
        <el-table-column prop="fbesFdpls" label="科目"/>
        <el-table-column prop="fbesStext" label="名称"/>
        <el-table-column prop="fbesFipls" label="明细科目"/>
        <el-table-column prop="fbesChtxt" label="明细名称"/>
        <el-table-column prop="fbesFtext" label="说明"/>
        <el-table-column prop="fbesAnval" label="预算金额"/>
        <el-table-column prop="fbesDmist" label="实际发生"/>
        <el-table-column prop="fbesBfaac" label="差异"/>
        <el-table-column prop="fbssAktiv" label="启用标记"/>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" :fullscreen="fullScreen">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="期间" prop="fbeLfgja">
              <el-select filterable clearable   v-model="form.fbeLfgja"  :placeholder="$t('btn.selectPrefix')+'期间'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="公司" prop="fbeBukrs">
              <el-select filterable clearable   v-model="form.fbeBukrs"  :placeholder="$t('btn.selectPrefix')+'公司'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="部门" prop="fbeAbtnr">
              <el-select filterable clearable   v-model="form.fbeAbtnr"  :placeholder="$t('btn.selectPrefix')+'部门'+$t('btn.selectSuffix')">
                <el-option
    ...
    <!-- 子表信息 -->
        ...
        <el-table :data="ficoBudgetExpenseSlvList" :row-class-name="rowFicoBudgetExpenseSlvIndex" @selection-change="handleFicoBudgetExpenseSlvSelectionChange" ref="FicoBudgetExpenseSlvRef">
          ...
          <el-table-column label="年月" align="center" prop="fbesLfmon">
            <template #default="scope">
              <el-input v-model="scope.row.fbesLfmon" :placeholder="$t('btn.enterPrefix')+'年月'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="科目" align="center" prop="fbesFdpls">
            <template #default="scope">
              <el-input v-model="scope.row.fbesFdpls" :placeholder="$t('btn.enterPrefix')+'科目'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="名称" align="center" prop="fbesStext">
            <template #default="scope">
              <el-input v-model="scope.row.fbesStext" :placeholder="$t('btn.enterPrefix')+'名称'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="明细科目" align="center" prop="fbesFipls">
            <template #default="scope">
              <el-input v-model="scope.row.fbesFipls" :placeholder="$t('btn.enterPrefix')+'明细科目'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="明细名称" align="center" prop="fbesChtxt">
            <template #default="scope">
              <el-input v-model="scope.row.fbesChtxt" :placeholder="$t('btn.enterPrefix')+'明细名称'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="fbesFtext">
            <template #default="scope">
              <el-input v-model="scope.row.fbesFtext" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="预算金额" align="center" prop="fbesAnval" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.fbesAnval" controls-position="right" :placeholder="$t('btn.enterPrefix')+'预算金额'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="实际发生" align="center" prop="fbesDmist" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.fbesDmist" controls-position="right" :placeholder="$t('btn.enterPrefix')+'实际发生'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="差异" align="center" prop="fbesBfaac" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.fbesBfaac" controls-position="right" :placeholder="$t('btn.enterPrefix')+'差异'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="启用标记" align="center" prop="fbssAktiv" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.fbssAktiv" controls-position="right" :placeholder="$t('btn.enterPrefix')+'启用标记'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
const columns = ref([
  ...
  { visible: true, prop: 'fbeLfgja', label: '期间' },
  { visible: true, prop: 'fbeBukrs', label: '公司' },
  { visible: true, prop: 'fbeAbtnr', label: '部门' },
