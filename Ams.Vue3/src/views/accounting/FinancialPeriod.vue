  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="财年" prop="fpFinancialYear">
        <el-input v-model="queryParams.fpFinancialYear" :placeholder="$t('btn.enterSearchPrefix')+'财年'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
        </el-col>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['fico:financialperiod:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['fico:financialperiod:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/FinancialPeriod/importTemplate"
                  importUrl="/Accounting/FinancialPeriod/importData"
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="fpFinancialYear" label="财年" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fpFinancialYear')"/>
      <el-table-column prop="fpYearMonth" label="年月" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('fpYearMonth')"/>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="财年" prop="fpFinancialYear">
              <el-input v-model="form.fpFinancialYear" :placeholder="$t('btn.enterPrefix')+'财年'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
<script setup name="financialperiod">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listFinancialPeriod,
 addFinancialPeriod, delFinancialPeriod, 
 updateFinancialPeriod,getFinancialPeriod, 
 } 
from '@/api/accounting/financialperiod.js'
...
function getList(){
  loading.value = true
  listFinancialPeriod(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
const state = reactive({
  ...
  rules: {
    ...
    fpFinancialYear: [{ required: true, message: "财年"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    fpYearMonth: [{ required: true, message: "年月"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
...
function handleUpdate(row) {
  ...
  getFinancialPeriod(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.fpSfid != undefined && opertype.value === 2) {
        updateFinancialPeriod(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addFinancialPeriod(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/FinancialPeriod/export', { ...queryParams })
    })
}
