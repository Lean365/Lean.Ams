  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="利润中心" prop="prctr">
        <el-input v-model="queryParams.prctr" :placeholder="$t('btn.enterSearchPrefix')+'利润中心'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="控制范围" prop="kokrs">
        <el-select filterable clearable   v-model="queryParams.kokrs" :placeholder="$t('btn.selectSearchPrefix')+'控制范围'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="名称" prop="ktext">
        <el-input v-model="queryParams.ktext" :placeholder="$t('btn.enterSearchPrefix')+'名称'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Accounting/ProfitCenter/importTemplate"
                  importUrl="/Accounting/ProfitCenter/importData"
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="prctr" label="利润中心" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('prctr')"/>
      ...
      <el-table-column prop="verak" label="负责人" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('verak')"/>
      ...
      <el-table-column prop="ktext" label="名称" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('ktext')"/>
      <el-table-column prop="ltext" label="长文本" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('ltext')"/>
      <el-table-column prop="mctext" label="利润中心说明" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mctext')"/>
      <el-table-column prop="khinr" label="利润中心组" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('khinr')"/>
      <el-table-column prop="etype" label="权益类型" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('etype')"/>
      <el-table-column prop="txjcd" label="地区税务代码" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('txjcd')"/>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="控制范围" prop="kokrs">
              <el-select filterable clearable   v-model="form.kokrs"  :placeholder="$t('btn.selectPrefix')+'控制范围'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="有效截至日期" prop="datbi">
              <el-date-picker v-model="form.datbi" type="datetime" :teleported="false" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </el-form-item>
          </el-col>
            
          <el-col :lg="12">
            <el-form-item label="部门" prop="abtei">
              <el-select filterable clearable   v-model="form.abtei"  :placeholder="$t('btn.selectPrefix')+'部门'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="货币" prop="waers">
              <el-select filterable clearable   v-model="form.waers"  :placeholder="$t('btn.selectPrefix')+'货币'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="名称" prop="ktext">
              <el-input v-model="form.ktext" :placeholder="$t('btn.enterPrefix')+'名称'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="利润中心说明" prop="mctext">
              <el-input v-model="form.mctext" :placeholder="$t('btn.enterPrefix')+'利润中心说明'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="权益类型" prop="etype">
              <el-input v-model="form.etype" :placeholder="$t('btn.enterPrefix')+'权益类型'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="锁定标记" prop="lockInd">
              <el-input-number v-model.number="form.lockInd" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'锁定标记'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="状态" prop="tstate">
              <el-radio-group v-model="form.tstate">
                <el-radio v-for="item in options.sys_activate_flag" :key="item.dictValue" :value="parseInt(item.dictValue)">
                  {{item.dictLabel}}
                </el-radio>
              </el-radio-group>
            </el-form-item>
...
<script setup name="profitcenter">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listProfitCenter,
 addProfitCenter, delProfitCenter, 
 updateProfitCenter,getProfitCenter, 
 } 
from '@/api/accounting/profitcenter.js'
...
//字段显示控制
const columns = ref([
  { visible: true, prop: 'fpSfid', label: 'ID' },
  ...
  { visible: false, prop: 'waers', label: '货币' },
  { visible: false, prop: 'ktext', label: '名称' },
  { visible: false, prop: 'ltext', label: '长文本' },
  { visible: false, prop: 'mctext', label: '利润中心说明' },
  { visible: false, prop: 'khinr', label: '利润中心组' },
  { visible: false, prop: 'etype', label: '权益类型' },
  { visible: false, prop: 'txjcd', label: '地区税务代码' },
  { visible: false, prop: 'lockInd', label: '锁定标记' },
  { visible: false, prop: 'tstate', label: '状态' },
...
function getList(){
  ...
  loading.value = true
  listProfitCenter(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
const state = reactive({
  ...
  rules: {
    ...
    datbi: [{ required: true, message: "有效截至日期"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    ktext: [{ required: true, message: "名称"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    ltext: [{ required: true, message: "长文本"+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
    lockInd: [{ required: true, message: "锁定标记"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    tstate: [{ required: true, message: "状态"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.fpSfid != undefined && opertype.value === 2) {
        updateProfitCenter(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addProfitCenter(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Accounting/ProfitCenter/export', { ...queryParams })
    })
}
