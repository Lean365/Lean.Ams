          ...
          <el-row :gutter="10" class="mb8">
            <el-col :lg="24">
              ...
              <el-form-item label="审核" prop="mh015">
                <el-radio-group v-model="queryParams.mh015">
...
<script setup name="ficobudgetexpense">
  ...
  const columns = ref([
    ...
    { visible: true, prop: 'mh010', label: '说明' },
    { visible: true, prop: 'mh011', label: '预算金额' },
    { visible: true, prop: 'mh012', label: '实际发生' },
    { visible: false, prop: 'mh013', label: '差异' },
    { visible: false, prop: 'mh014', label: '启用标记' },
    { visible: true, prop: 'mh015', label: '审核' },
  ...
  //const arrayIncludesValue = computed(() => cropDta.value.includes(deptIdSelect.value));
  // 添加按钮操作
  function handleAdd() {
    console.log(deptIdSelect.value)
    reset();
    open.value = true
    title.value = proxy.$t('btn.add') + " " + '费用预算'
    opertype.value = 1
    form.value.deptId = deptIdSelect
