...
<script setup name="monthlyecbar">
  import * as echarts from 'echarts'

  import {
    getMonthlyEcCount, getEcCount, getMonthlyEcBar
  }
  ...
  const getBarList = () => {
    ...
    getMonthlyEcBar(queryParams).then(response => {
      MapList = response.data;
      //console.log("柱状图数据：" + MapList);         // 确认数据有获取到
      let obj = eval(MapList);
      for (let i = 0; i < obj.length; i++) {
        ecDate.push(MapList[i].ecDate);
        ecCount.push(MapList[i].ecCount);
      }
      //动态插入数据时，数据在这里设置更新
      chart.setOption({
        xAxis: {
