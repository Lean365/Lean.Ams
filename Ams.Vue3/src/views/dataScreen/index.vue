...
import { getMonthlyInventoryAmount } from '@/api/statistics/inventory.js'
import { getMonthlySalesAmount } from '@/api/statistics/sales.js'
import { getMonthlyOutputQty } from '@/api/statistics/assyoutput.js'

const gcia = ref()

function getInventoryAmount() {
  return new Promise((resolve, reject) => {
    getMonthlyInventoryAmount().then((res) => {
      //
      const { code, data } = res
      if (code == 200) {
        console.log('在库', data)
...
function getMonthlySales() {
  return new Promise((resolve, reject) => {
    getMonthlySalesAmount().then((res) => {
      //
      const { code, data } = res
      if (code == 200) {
        const arr = data //.split(",");
