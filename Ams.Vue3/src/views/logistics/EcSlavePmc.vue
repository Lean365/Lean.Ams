          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/EcSlavePmc/importTemplate"
                  importUrl="/Logistics/EcSlavePmc/importData"
...
<script setup name="ecslavepmc">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listEcSlavePmc,
 addEcSlavePmc, delEcSlavePmc, 
 updateEcSlavePmc,getEcSlavePmc, 
 } 
from '@/api/logistics/ecslavepmc.js'
...
function getList(){
  ...
  loading.value = true
  listEcSlavePmc(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.pmcSfid != undefined && opertype.value === 2) {
        updateEcSlavePmc(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addEcSlavePmc(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Logistics/EcSlavePmc/export', { ...queryParams })
    })
}
