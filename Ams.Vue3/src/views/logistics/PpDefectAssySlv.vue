<!--
 * @Descripttion: 不良明细/pp_defect_assy_slv
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:defectassyslv:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:defectassyslv:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:defectassyslv:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:defectassyslv:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpDefectAssySlv/importTemplate"
                  importUrl="/Logistics/PpDefectAssySlv/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:defectassyslv:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column prop="mfa003" label="不良类别" align="center" v-if="columns.showColumn('mfa003')">
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:defectassyslv:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:defectassyslv:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="不良类别" prop="mfa003">
              <el-select filterable clearable   v-model="form.mfa003"  :placeholder="$t('btn.selectPrefix')+'不良类别'+$t('btn.selectSuffix')">
                <el-option
...
<script setup name="ppdefectassyslv">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpDefectAssySlv,
 addPpDefectAssySlv, delPpDefectAssySlv, 
 updatePpDefectAssySlv,getPpDefectAssySlv, 
 } 
from '@/api/logistics/ppdefectassyslv.js'
...
const columns = ref([
  ...
  { visible: true, prop: 'mfa003', label: '不良类别' },
...
//API获取从不良明细/pp_defect_assy_slv表记录数据
function getList(){
  loading.value = true
  listPpDefectAssySlv(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleUpdate(row) {
  ...
  getPpDefectAssySlv(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.id != undefined && opertype.value === 2) {
        updatePpDefectAssySlv(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpDefectAssySlv(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Logistics/PpDefectAssySlv/export', { ...queryParams })
    })
}
