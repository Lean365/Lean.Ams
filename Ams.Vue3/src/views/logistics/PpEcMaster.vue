  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="设变No." prop="emEcNo">
        <el-input v-model="queryParams.emEcNo" :placeholder="$t('btn.enter')+'设变No.'" />
      </el-form-item>
      <el-form-item label="状态" prop="emEcStatus">
        <el-radio-group v-model="queryParams.emEcStatus">
      ...
      <el-form-item label="标题" prop="emEcTitle">
        <el-input v-model="queryParams.emEcTitle" :placeholder="$t('btn.enter')+'标题'" />
      </el-form-item>
      ...
      <el-form-item label="管理区分" prop="emEcManageCategory">
        <el-select filterable clearable   v-model="queryParams.emEcManageCategory" :placeholder="$t('btn.select')+'管理区分'">
          <el-option v-for="item in   options.app_ec_mgtype " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>          
          </el-option>
        </el-select>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button type="add" v-hasPermi="['pp:ecmaster:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="edit" :disabled="single" v-hasPermi="['pp:ecmaster:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmaster:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button type="export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmaster:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="650"
      ...
      <el-table-column prop="emEcStatus" label="状态" align="center" v-if="columns.showColumn('emEcStatus')">
        <template #default="scope">
          <dict-tag :options=" options.sys_yes_no " :value="scope.row.emEcStatus"  />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="emEcManageCategory" label="管理区分" align="center" v-if="columns.showColumn('emEcManageCategory')">
        <template #default="scope">
          <dict-tag :options=" options.app_ec_mgtype " :value="scope.row.emEcManageCategory"  />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button type="edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmaster:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button type="delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmaster:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" :fullscreen="fullScreen">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="SFID" prop="emSFID">
              <el-input v-model.number="form.emSFID" :placeholder="$t('btn.enter')+'SFID'" :disabled="opertype != 1"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="管理区分" prop="emEcManageCategory">
              <el-select filterable clearable   v-model="form.emEcManageCategory"  :placeholder="$t('btn.select')+'管理区分'">
                <el-option
                  v-for="item in  options.app_ec_mgtype" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-col :lg="12">
            <el-form-item label="输入部门" prop="emEcImpDept">
              <el-input v-model="form.emEcImpDept" :placeholder="$t('btn.enter')+'输入部门'" />
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="add" icon="Plus" @click="handleAddPpEcSlave">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="delete" icon="Delete" @click="handleDeletePpEcSlave">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlaveList" :row-class-name="rowPpEcSlaveIndex" @selection-change="handlePpEcSlaveSelectionChange" ref="PpEcSlaveRef">
          ...
          <el-table-column label="输入日" align="center" prop="esEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="esModel">
            <template #default="scope">
              <el-input v-model="scope.row.esModel" :placeholder="$t('btn.enter')+'机种'" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="esItem">
            <template #default="scope">
              <el-input v-model="scope.row.esItem" :placeholder="$t('btn.enter')+'物料'" />
            </template>
          </el-table-column>
          <el-table-column label="子物料" align="center" prop="esSubItem">
            <template #default="scope">
              <el-input v-model="scope.row.esSubItem" :placeholder="$t('btn.enter')+'子物料'" />
            </template>
          </el-table-column>
          <el-table-column label="旧物料" align="center" prop="esOldItem">
            <template #default="scope">
              <el-input v-model="scope.row.esOldItem" :placeholder="$t('btn.enter')+'旧物料'" />
            </template>
          </el-table-column>
          <el-table-column label="旧文本" align="center" prop="esOldItemText">
            <template #default="scope">
              <el-input v-model="scope.row.esOldItemText" :placeholder="$t('btn.enter')+'旧文本'" />
            </template>
          </el-table-column>
          <el-table-column label="用量" align="center" prop="esOldUsageQty" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.esOldUsageQty" controls-position="right" :placeholder="$t('btn.enter')+'用量'" />
            </template>
          </el-table-column>
          <el-table-column label="位置" align="center" prop="esOldSetLoc">
            <template #default="scope">
              <el-input v-model="scope.row.esOldSetLoc" :placeholder="$t('btn.enter')+'位置'" />
            </template>
          </el-table-column>
          <el-table-column label="新物料" align="center" prop="esNewItem">
            <template #default="scope">
              <el-input v-model="scope.row.esNewItem" :placeholder="$t('btn.enter')+'新物料'" />
            </template>
          </el-table-column>
          <el-table-column label="新文本" align="center" prop="esNewItemText">
            <template #default="scope">
              <el-input v-model="scope.row.esNewItemText" :placeholder="$t('btn.enter')+'新文本'" />
            </template>
          </el-table-column>
          <el-table-column label="用量" align="center" prop="esNewUsageQty" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.esNewUsageQty" controls-position="right" :placeholder="$t('btn.enter')+'用量'" />
            </template>
          </el-table-column>
          <el-table-column label="位置" align="center" prop="esNewSetLoc">
            <template #default="scope">
              <el-input v-model="scope.row.esNewSetLoc" :placeholder="$t('btn.enter')+'位置'" />
            </template>
          </el-table-column>
          <el-table-column label="bom番号" align="center" prop="esBomNo">
            <template #default="scope">
              <el-input v-model="scope.row.esBomNo" :placeholder="$t('btn.enter')+'bom番号'" />
            </template>
          </el-table-column>
          <el-table-column label="互换" align="center" prop="esChange">
            <template #default="scope">
              <el-input v-model="scope.row.esChange" :placeholder="$t('btn.enter')+'互换'" />
            </template>
          </el-table-column>
          <el-table-column label="区分" align="center" prop="eslocal">
            <template #default="scope">
              <el-input v-model="scope.row.eslocal" :placeholder="$t('btn.enter')+'区分'" />
            </template>
          </el-table-column>
          <el-table-column label="指示" align="center" prop="esnote">
            <template #default="scope">
              <el-input v-model="scope.row.esnote" :placeholder="$t('btn.enter')+'指示'" />
            </template>
          </el-table-column>
          <el-table-column label="旧品处理" align="center" prop="esOldProcess">
            <template #default="scope">
              <el-input v-model="scope.row.esOldProcess" :placeholder="$t('btn.enter')+'旧品处理'" />
            </template>
          </el-table-column>
          <el-table-column label="bom日期" align="center" prop="esBomDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esBomDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="实施部门" prop="emEcImpDept">
            <template #default="scope">
              <el-select v-model="scope.row.emEcImpDept" :placeholder="$t('btn.enter')+'实施部门'">
                <el-option
          ...
          <el-table-column label="采购类型" prop="esPurType">
            <template #default="scope">
              <el-select v-model="scope.row.esPurType" :placeholder="$t('btn.enter')+'采购类型'">
                <el-option
          ...
          <el-table-column label="仓库" prop="esSloc">
            <template #default="scope">
              <el-select v-model="scope.row.esSloc" :placeholder="$t('btn.enter')+'仓库'">
                <el-option
          ...
          <el-table-column label="检验否" prop="esInsmk">
            <template #default="scope">
              <el-select v-model="scope.row.esInsmk" :placeholder="$t('btn.enter')+'检验否'">
                <el-option
                  v-for="item in options.sys_yes_no" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
                  :value="item.dictValue"></el-option>
              </el-select>
          ...
          <el-table-column label="工厂状态" prop="esMstae">
            <template #default="scope">
              <el-select v-model="scope.row.esMstae" :placeholder="$t('btn.enter')+'工厂状态'">
                <el-option
                  v-for="item in options.sys_yes_no" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="SOP" prop="esSopStae">
            <template #default="scope">
              <el-select v-model="scope.row.esSopStae" :placeholder="$t('btn.enter')+'SOP'">
                <el-option
                  v-for="item in options.sys_yes_no" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="旧品库存" align="center" prop="esOldCurrStock" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.esOldCurrStock" controls-position="right" :placeholder="$t('btn.enter')+'旧品库存'" />
            </template>
          </el-table-column>
          <el-table-column label="新品库存" align="center" prop="esNewCurrStock" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.esNewCurrStock" controls-position="right" :placeholder="$t('btn.enter')+'新品库存'" />
            </template>
          </el-table-column>
          <el-table-column label="采购登入日期" align="center" prop="esPurEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPurEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="供应商" align="center" prop="esSupplier">
            <template #default="scope">
              <el-input v-model="scope.row.esSupplier" :placeholder="$t('btn.enter')+'供应商'" />
            </template>
          </el-table-column>
          <el-table-column label="采购PO" align="center" prop="esPurOrder">
            <template #default="scope">
              <el-input v-model="scope.row.esPurOrder" :placeholder="$t('btn.enter')+'采购PO'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esPurNote">
            <template #default="scope">
              <el-input v-model="scope.row.esPurNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="采购新增" align="center" prop="esPurCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esPurCreator" :placeholder="$t('btn.enter')+'采购新增'" />
            </template>
          </el-table-column>
          <el-table-column label="采购新增日期" align="center" prop="esPurCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPurCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="采购修改" align="center" prop="esPurModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esPurModifier" :placeholder="$t('btn.enter')+'采购修改'" />
            </template>
          </el-table-column>
          <el-table-column label="采购修改日期" align="center" prop="esPurModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPurModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="生管登入日期" align="center" prop="esPmcEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPmcEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="预投入批次" align="center" prop="esPmcLot">
            <template #default="scope">
              <el-input v-model="scope.row.esPmcLot" :placeholder="$t('btn.enter')+'预投入批次'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esPmcMemo">
            <template #default="scope">
              <el-input v-model="scope.row.esPmcMemo" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="备注" align="center" prop="esPmcNote">
            <template #default="scope">
              <el-input v-model="scope.row.esPmcNote" :placeholder="$t('btn.enter')+'备注'" />
            </template>
          </el-table-column>
          <el-table-column label="生管新增" align="center" prop="esPmcCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esPmcCreator" :placeholder="$t('btn.enter')+'生管新增'" />
            </template>
          </el-table-column>
          <el-table-column label="生管新增日期" align="center" prop="esPmcCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPmcCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="生管修改" align="center" prop="esPmcModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esPmcModifier" :placeholder="$t('btn.enter')+'生管修改'" />
            </template>
          </el-table-column>
          <el-table-column label="生管修改日期" align="center" prop="esPmcModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPmcModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="IQC登入日期" align="center" prop="esIqcEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esIqcEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="检验订单" align="center" prop="esIqcOrder">
            <template #default="scope">
              <el-input v-model="scope.row.esIqcOrder" :placeholder="$t('btn.enter')+'检验订单'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esIqcNote">
            <template #default="scope">
              <el-input v-model="scope.row.esIqcNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="IQC新增" align="center" prop="esIqcCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esIqcCreator" :placeholder="$t('btn.enter')+'IQC新增'" />
            </template>
          </el-table-column>
          <el-table-column label="IQC新增日期" align="center" prop="esIqcCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esIqcCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="IQC修改" align="center" prop="esIqcModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esIqcModifier" :placeholder="$t('btn.enter')+'IQC修改'" />
            </template>
          </el-table-column>
          <el-table-column label="IQC修改日期" align="center" prop="esIqcModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esIqcModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="部管登入日期" align="center" prop="esMmEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esMmEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="领用批次" align="center" prop="esMmLot">
            <template #default="scope">
              <el-input v-model="scope.row.esMmLot" :placeholder="$t('btn.enter')+'领用批次'" />
            </template>
          </el-table-column>
          <el-table-column label="工单" align="center" prop="esMmMocNo">
            <template #default="scope">
              <el-input v-model="scope.row.esMmMocNo" :placeholder="$t('btn.enter')+'工单'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esMmNote">
            <template #default="scope">
              <el-input v-model="scope.row.esMmNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="EsMmCreator" align="center" prop="esMmCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esMmCreator" :placeholder="$t('btn.enter')+'EsMmCreator'" />
            </template>
          </el-table-column>
          <el-table-column label="EsMmCreateTime" align="center" prop="esMmCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esMmCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="EsMmModifier" align="center" prop="esMmModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esMmModifier" :placeholder="$t('btn.enter')+'EsMmModifier'" />
            </template>
          </el-table-column>
          <el-table-column label="EsMmModifyTime" align="center" prop="esMmModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esMmModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="制一登入日期" align="center" prop="esPpEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPpEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="班组" align="center" prop="esPpLine">
            <template #default="scope">
              <el-input v-model="scope.row.esPpLine" :placeholder="$t('btn.enter')+'班组'" />
            </template>
          </el-table-column>
          <el-table-column label="生产批次" align="center" prop="esPpLot">
            <template #default="scope">
              <el-input v-model="scope.row.esPpLot" :placeholder="$t('btn.enter')+'生产批次'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esPpNote">
            <template #default="scope">
              <el-input v-model="scope.row.esPpNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPpCreator" align="center" prop="esPpCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esPpCreator" :placeholder="$t('btn.enter')+'EsPpCreator'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPpCreateTime" align="center" prop="esPpCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPpCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="EsPpModifier" align="center" prop="esPpModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esPpModifier" :placeholder="$t('btn.enter')+'EsPpModifier'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPpModifyTime" align="center" prop="esPpModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPpModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="制二登入日期" align="center" prop="esPcbaEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPcbaEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="Pcba班组" align="center" prop="esPcbaLine">
            <template #default="scope">
              <el-input v-model="scope.row.esPcbaLine" :placeholder="$t('btn.enter')+'Pcba班组'" />
            </template>
          </el-table-column>
          <el-table-column label="Pcba批次" align="center" prop="esPcbaLot">
            <template #default="scope">
              <el-input v-model="scope.row.esPcbaLot" :placeholder="$t('btn.enter')+'Pcba批次'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esPcbaNote">
            <template #default="scope">
              <el-input v-model="scope.row.esPcbaNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPcbaCreator" align="center" prop="esPcbaCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esPcbaCreator" :placeholder="$t('btn.enter')+'EsPcbaCreator'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPcbaCreateTime" align="center" prop="esPcbaCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPcbaCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="EsPcbaModifier" align="center" prop="esPcbaModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esPcbaModifier" :placeholder="$t('btn.enter')+'EsPcbaModifier'" />
            </template>
          </el-table-column>
          <el-table-column label="EsPcbaModifyTime" align="center" prop="esPcbaModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esPcbaModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="QA确认日期" align="center" prop="esQaEntryDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esQaEntryDate" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="检验批次" align="center" prop="esQaLot">
            <template #default="scope">
              <el-input v-model="scope.row.esQaLot" :placeholder="$t('btn.enter')+'检验批次'" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="esQaNote">
            <template #default="scope">
              <el-input v-model="scope.row.esQaNote" :placeholder="$t('btn.enter')+'说明'" />
            </template>
          </el-table-column>
          <el-table-column label="EsQaCreator" align="center" prop="esQaCreator">
            <template #default="scope">
              <el-input v-model="scope.row.esQaCreator" :placeholder="$t('btn.enter')+'EsQaCreator'" />
            </template>
          </el-table-column>
          <el-table-column label="EsQaCreateTime" align="center" prop="esQaCreateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esQaCreateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="EsQaModifier" align="center" prop="esQaModifier">
            <template #default="scope">
              <el-input v-model="scope.row.esQaModifier" :placeholder="$t('btn.enter')+'EsQaModifier'" />
            </template>
          </el-table-column>
          <el-table-column label="EsQaModifyTime" align="center" prop="esQaModifyTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.esQaModifyTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="备注" align="center" prop="remark">
            <template #default="scope">
              <el-input v-model="scope.row.remark" :placeholder="$t('btn.enter')+'备注'" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="创建时间" align="center" prop="createTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.createTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
          ...
          <el-table-column label="更新时间" align="center" prop="updateTime">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.updateTime" type="date" :placeholder="$t('btn.select')"></el-date-picker>
            </template>
          </el-table-column>
        </el-table>
      </el-form>
...
const state = reactive({
  ...
  form: {},
  rules: {
    emSFID: [{ required: true, message: "SFID不能为空", trigger: "blur" }],
    ...
    emEcImpDept: [{ required: true, message: "输入部门不能为空", trigger: "blur"     }],
