<!--
 * @Descripttion: 制一/pp_ec_master_assy
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmasterassy:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmasterassy:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmasterassy:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmasterassy:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterAssy/importTemplate"
                  importUrl="/Logistics/PpEcMasterAssy/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmasterassy:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmasterassy:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmasterassy:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlaveAssyList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="assySfid" label="ID"/>
        <el-table-column prop="assyParentSfid" label="父ID"/>
        <el-table-column prop="assyEcNo" label="设变No."/>
        <el-table-column prop="assyModel" label="机种"/>
        <el-table-column prop="assyItem" label="物料"/>
        <el-table-column prop="assyImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.assyImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="assyEnteredDate" label="制一登入日期"/>
        <el-table-column prop="assyLine" label="班组">
          <template #default="scope">
            <dict-tag :options=" options.sql_line_list " :value="scope.row.assyLine"  />
          </template>
        </el-table-column>
        <el-table-column prop="assyPutLot" label="生产批次"/>
        <el-table-column prop="assyNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">组立</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlaveAssy">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlaveAssy">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlaveAssyList" :row-class-name="rowPpEcSlaveAssyIndex" @selection-change="handlePpEcSlaveAssySelectionChange" ref="PpEcSlaveAssyRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="assySfid">
            <template #default="scope">
              <el-input v-model="scope.row.assySfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="assyEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.assyEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="assyModel">
            <template #default="scope">
              <el-input v-model="scope.row.assyModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="assyItem">
            <template #default="scope">
              <el-input v-model="scope.row.assyItem" :placeholder="$t('btn.enterPrefix')+'物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="实施标记" prop="assyImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.assyImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="制一登入日期" align="center" prop="assyEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.assyEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="班组" prop="assyLine">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.assyLine" :placeholder="$t('btn.enterPrefix')+'班组'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="生产批次" align="center" prop="assyPutLot">
            <template #default="scope">
              <el-input v-model="scope.row.assyPutLot" :placeholder="$t('btn.enterPrefix')+'生产批次'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="assyNote">
            <template #default="scope">
              <el-input v-model="scope.row.assyNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmasterassy">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterAssy,
 addPpEcMasterAssy, delPpEcMasterAssy, 
 updatePpEcMasterAssy,getPpEcMasterAssy, 
 } 
from '@/api/logistics/ppecmasterassy.js'
...
//API获取从制一/pp_ec_master_assy表记录数据
function getList(){
  ...
  loading.value = true
  listPpEcMasterAssy(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'制一'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterAssy(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '制一'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlaveAssyList.value = res.data.ppEcSlaveAssyNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlaveAssyNav = ppEcSlaveAssyList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterAssy(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterAssy(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<制一.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterAssy/export', { ...queryParams })
    })
}

/*********************组立子表信息*************************/
const ppEcSlaveAssyList = ref([])
const checkedPpEcSlaveAssy = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** 组立序号 */
function rowPpEcSlaveAssyIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** 组立添加按钮操作 */
function handleAddPpEcSlaveAssy() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.assySfid = null;
  //obj.assyEcNo = null;
  //obj.assyModel = null;
  //obj.assyItem = null;
  //obj.assyImplStated = null;
  //obj.assyEnteredDate = null;
  //obj.assyLine = null;
  //obj.assyPutLot = null;
  //obj.assyNote = null;
  //obj.remark = null;
  ...
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlaveAssyList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlaveAssySelectionChange(selection) {
  checkedPpEcSlaveAssy.value = selection.map(item => item.index)
}

/** 组立删除按钮操作 */
function handleDeletePpEcSlaveAssy() {
  if(checkedPpEcSlaveAssy.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的组立数据')
  } else {
    const PpEcSlaveAssys = ppEcSlaveAssyList.value;
    const checkedPpEcSlaveAssys = checkedPpEcSlaveAssy.value;
    ppEcSlaveAssyList.value = PpEcSlaveAssys.filter(function(item) {
      return checkedPpEcSlaveAssys.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterAssy(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlaveAssyList.value = data.ppEcSlaveAssyNav
    }
  })
}
