<!--
 * @Descripttion: 品管/pp_ec_master_fqc
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmasterfqc:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmasterfqc:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmasterfqc:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmasterfqc:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterFqc/importTemplate"
                  importUrl="/Logistics/PpEcMasterFqc/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmasterfqc:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmasterfqc:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmasterfqc:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlaveFqcList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="fqcSfid" label="ID"/>
        <el-table-column prop="fqcParentSfid" label="父ID"/>
        <el-table-column prop="fqcEcNo" label="设变No."/>
        <el-table-column prop="fqcModel" label="机种"/>
        <el-table-column prop="fqcItem" label="物料"/>
        <el-table-column prop="fqcImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.fqcImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="fqcEnteredDate" label="QA确认日期"/>
        <el-table-column prop="fqcImplLot" label="投入批次"/>
        <el-table-column prop="fqcNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">品管</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlaveFqc">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlaveFqc">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlaveFqcList" :row-class-name="rowPpEcSlaveFqcIndex" @selection-change="handlePpEcSlaveFqcSelectionChange" ref="PpEcSlaveFqcRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="fqcSfid">
            <template #default="scope">
              <el-input v-model="scope.row.fqcSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="fqcEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.fqcEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="fqcModel">
            <template #default="scope">
              <el-input v-model="scope.row.fqcModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="fqcItem">
            <template #default="scope">
              <el-input v-model="scope.row.fqcItem" :placeholder="$t('btn.enterPrefix')+'物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="实施标记" prop="fqcImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.fqcImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="QA确认日期" align="center" prop="fqcEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.fqcEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="投入批次" align="center" prop="fqcImplLot">
            <template #default="scope">
              <el-input v-model="scope.row.fqcImplLot" :placeholder="$t('btn.enterPrefix')+'投入批次'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="fqcNote">
            <template #default="scope">
              <el-input v-model="scope.row.fqcNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmasterfqc">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterFqc,
 addPpEcMasterFqc, delPpEcMasterFqc, 
 updatePpEcMasterFqc,getPpEcMasterFqc, 
 } 
from '@/api/logistics/ppecmasterfqc.js'
...
//API获取从品管/pp_ec_master_fqc表记录数据
function getList(){
  ...
  loading.value = true
  listPpEcMasterFqc(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'品管'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterFqc(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '品管'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlaveFqcList.value = res.data.ppEcSlaveFqcNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlaveFqcNav = ppEcSlaveFqcList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterFqc(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterFqc(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<品管.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterFqc/export', { ...queryParams })
    })
}

/*********************品管子表信息*************************/
const ppEcSlaveFqcList = ref([])
const checkedPpEcSlaveFqc = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** 品管序号 */
function rowPpEcSlaveFqcIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** 品管添加按钮操作 */
function handleAddPpEcSlaveFqc() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.fqcSfid = null;
  //obj.fqcEcNo = null;
  //obj.fqcModel = null;
  //obj.fqcItem = null;
  //obj.fqcImplStated = null;
  //obj.fqcEnteredDate = null;
  //obj.fqcImplLot = null;
  //obj.fqcNote = null;
  //obj.remark = null;
  //obj.createBy = null;
  ...
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlaveFqcList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlaveFqcSelectionChange(selection) {
  checkedPpEcSlaveFqc.value = selection.map(item => item.index)
}

/** 品管删除按钮操作 */
function handleDeletePpEcSlaveFqc() {
  if(checkedPpEcSlaveFqc.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的品管数据')
  } else {
    const PpEcSlaveFqcs = ppEcSlaveFqcList.value;
    const checkedPpEcSlaveFqcs = checkedPpEcSlaveFqc.value;
    ppEcSlaveFqcList.value = PpEcSlaveFqcs.filter(function(item) {
      return checkedPpEcSlaveFqcs.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterFqc(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlaveFqcList.value = data.ppEcSlaveFqcNav
    }
  })
}
