<!--
 * @Descripttion: 受检/pp_ec_master_iqc
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmasteriqc:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmasteriqc:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmasteriqc:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmasteriqc:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterIqc/importTemplate"
                  importUrl="/Logistics/PpEcMasterIqc/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmasteriqc:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmasteriqc:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmasteriqc:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlaveIqcList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="iqcSfid" label="ID"/>
        <el-table-column prop="iqcParentSfid" label="父ID"/>
        <el-table-column prop="iqcEcNo" label="设变No."/>
        <el-table-column prop="iqcModel" label="机种"/>
        <el-table-column prop="iqcNewItem" label="新物料"/>
        <el-table-column prop="iqcNewItemText" label="新文本"/>
        <el-table-column prop="iqcNewCurrStock" label="新品库存"/>
        <el-table-column prop="iqcPurType" label="采购类型">
          <template #default="scope">
            <dict-tag :options=" options.sys_pur_type " :value="scope.row.iqcPurType"  />
          </template>
        </el-table-column>
        <el-table-column prop="iqcPurGroup" label="采购组">
          <template #default="scope">
            <dict-tag :options=" options.sys_pur_group " :value="scope.row.iqcPurGroup"  />
          </template>
        </el-table-column>
        <el-table-column prop="iqcInsmk" label="检验否">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.iqcInsmk"  />
          </template>
        </el-table-column>
        <el-table-column prop="iqcImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.iqcImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="iqcEnteredDate" label="IQC登入日期"/>
        <el-table-column prop="iqcPurOrder" label="检验订单"/>
        <el-table-column prop="iqcNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">Iqc设变</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlaveIqc">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlaveIqc">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlaveIqcList" :row-class-name="rowPpEcSlaveIqcIndex" @selection-change="handlePpEcSlaveIqcSelectionChange" ref="PpEcSlaveIqcRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="iqcSfid">
            <template #default="scope">
              <el-input v-model="scope.row.iqcSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="iqcEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.iqcEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="iqcModel">
            <template #default="scope">
              <el-input v-model="scope.row.iqcModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="新物料" align="center" prop="iqcNewItem">
            <template #default="scope">
              <el-input v-model="scope.row.iqcNewItem" :placeholder="$t('btn.enterPrefix')+'新物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="新文本" align="center" prop="iqcNewItemText">
            <template #default="scope">
              <el-input v-model="scope.row.iqcNewItemText" :placeholder="$t('btn.enterPrefix')+'新文本'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="新品库存" align="center" prop="iqcNewCurrStock" width="140">
            <template #default="scope">
              <el-input-number v-model="scope.row.iqcNewCurrStock" controls-position="right" :placeholder="$t('btn.enterPrefix')+'新品库存'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="采购类型" prop="iqcPurType">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.iqcPurType" :placeholder="$t('btn.enterPrefix')+'采购类型'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="采购组" prop="iqcPurGroup">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.iqcPurGroup" :placeholder="$t('btn.enterPrefix')+'采购组'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="检验否" prop="iqcInsmk">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.iqcInsmk" :placeholder="$t('btn.enterPrefix')+'检验否'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="实施标记" prop="iqcImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.iqcImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
                  v-for="item in options.sys_flag_list" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="IQC登入日期" align="center" prop="iqcEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.iqcEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="检验订单" align="center" prop="iqcPurOrder">
            <template #default="scope">
              <el-input v-model="scope.row.iqcPurOrder" :placeholder="$t('btn.enterPrefix')+'检验订单'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="iqcNote">
            <template #default="scope">
              <el-input v-model="scope.row.iqcNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmasteriqc">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterIqc,
 addPpEcMasterIqc, delPpEcMasterIqc, 
 updatePpEcMasterIqc,getPpEcMasterIqc, 
 } 
from '@/api/logistics/ppecmasteriqc.js'
...
//API获取从受检/pp_ec_master_iqc表记录数据
function getList(){
  ...
  loading.value = true
  listPpEcMasterIqc(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'受检'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterIqc(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '受检'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlaveIqcList.value = res.data.ppEcSlaveIqcNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlaveIqcNav = ppEcSlaveIqcList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterIqc(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterIqc(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<受检.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterIqc/export', { ...queryParams })
    })
}

/*********************Iqc设变子表信息*************************/
const ppEcSlaveIqcList = ref([])
const checkedPpEcSlaveIqc = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** Iqc设变序号 */
function rowPpEcSlaveIqcIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** Iqc设变添加按钮操作 */
function handleAddPpEcSlaveIqc() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.iqcSfid = null;
  //obj.iqcEcNo = null;
  //obj.iqcModel = null;
  //obj.iqcNewItem = null;
  //obj.iqcNewItemText = null;
  //obj.iqcNewCurrStock = null;
  //obj.iqcPurType = null;
  //obj.iqcPurGroup = null;
  //obj.iqcInsmk = null;
  //obj.iqcImplStated = null;
  //obj.iqcEnteredDate = null;
  //obj.iqcPurOrder = null;
  //obj.iqcNote = null;
  //obj.remark = null;
  //obj.createBy = null;
  //obj.createTime = null;
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlaveIqcList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlaveIqcSelectionChange(selection) {
  checkedPpEcSlaveIqc.value = selection.map(item => item.index)
}

/** Iqc设变删除按钮操作 */
function handleDeletePpEcSlaveIqc() {
  if(checkedPpEcSlaveIqc.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的Iqc设变数据')
  } else {
    const PpEcSlaveIqcs = ppEcSlaveIqcList.value;
    const checkedPpEcSlaveIqcs = checkedPpEcSlaveIqc.value;
    ppEcSlaveIqcList.value = PpEcSlaveIqcs.filter(function(item) {
      return checkedPpEcSlaveIqcs.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterIqc(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlaveIqcList.value = data.ppEcSlaveIqcNav
    }
  })
}
