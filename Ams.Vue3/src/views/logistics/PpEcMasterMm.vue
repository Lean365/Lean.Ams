<!--
 * @Descripttion: 部管/pp_ec_master_mm
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmastermm:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmastermm:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmastermm:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmastermm:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterMm/importTemplate"
                  importUrl="/Logistics/PpEcMasterMm/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmastermm:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmastermm:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmastermm:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlaveMmList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="mmSfid" label="ID"/>
        <el-table-column prop="mmParentSfid" label="父ID"/>
        <el-table-column prop="mmEcNo" label="设变No."/>
        <el-table-column prop="mmModel" label="机种"/>
        <el-table-column prop="mmItem" label="物料"/>
        <el-table-column prop="mmImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.mmImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="mmEnteredDate" label="部管登入日期"/>
        <el-table-column prop="mmLot" label="领用批次"/>
        <el-table-column prop="mmProOrder" label="工单"/>
        <el-table-column prop="mmNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">Mm设变</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlaveMm">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlaveMm">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlaveMmList" :row-class-name="rowPpEcSlaveMmIndex" @selection-change="handlePpEcSlaveMmSelectionChange" ref="PpEcSlaveMmRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="mmSfid">
            <template #default="scope">
              <el-input v-model="scope.row.mmSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="mmEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.mmEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="mmModel">
            <template #default="scope">
              <el-input v-model="scope.row.mmModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="mmItem">
            <template #default="scope">
              <el-input v-model="scope.row.mmItem" :placeholder="$t('btn.enterPrefix')+'物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="实施标记" prop="mmImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.mmImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="部管登入日期" align="center" prop="mmEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.mmEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="领用批次" align="center" prop="mmLot">
            <template #default="scope">
              <el-input v-model="scope.row.mmLot" :placeholder="$t('btn.enterPrefix')+'领用批次'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="工单" align="center" prop="mmProOrder">
            <template #default="scope">
              <el-input v-model="scope.row.mmProOrder" :placeholder="$t('btn.enterPrefix')+'工单'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="mmNote">
            <template #default="scope">
              <el-input v-model="scope.row.mmNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmastermm">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterMm,
 addPpEcMasterMm, delPpEcMasterMm, 
 updatePpEcMasterMm,getPpEcMasterMm, 
 } 
from '@/api/logistics/ppecmastermm.js'
...
function getList(){
  ...
  loading.value = true
  listPpEcMasterMm(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'部管'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterMm(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '部管'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlaveMmList.value = res.data.ppEcSlaveMmNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlaveMmNav = ppEcSlaveMmList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterMm(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterMm(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<部管.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterMm/export', { ...queryParams })
    })
}

/*********************Mm设变子表信息*************************/
const ppEcSlaveMmList = ref([])
const checkedPpEcSlaveMm = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** Mm设变序号 */
function rowPpEcSlaveMmIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** Mm设变添加按钮操作 */
function handleAddPpEcSlaveMm() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.mmSfid = null;
  //obj.mmEcNo = null;
  //obj.mmModel = null;
  //obj.mmItem = null;
  //obj.mmImplStated = null;
  //obj.mmEnteredDate = null;
  //obj.mmLot = null;
  //obj.mmProOrder = null;
  //obj.mmNote = null;
  //obj.remark = null;
  ...
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlaveMmList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlaveMmSelectionChange(selection) {
  checkedPpEcSlaveMm.value = selection.map(item => item.index)
}

/** Mm设变删除按钮操作 */
function handleDeletePpEcSlaveMm() {
  if(checkedPpEcSlaveMm.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的Mm设变数据')
  } else {
    const PpEcSlaveMms = ppEcSlaveMmList.value;
    const checkedPpEcSlaveMms = checkedPpEcSlaveMm.value;
    ppEcSlaveMmList.value = PpEcSlaveMms.filter(function(item) {
      return checkedPpEcSlaveMms.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterMm(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlaveMmList.value = data.ppEcSlaveMmNav
    }
  })
}
