<!--
 * @Descripttion: 制二/pp_ec_master_pcba
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmasterpcba:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmasterpcba:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmasterpcba:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmasterpcba:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterPcba/importTemplate"
                  importUrl="/Logistics/PpEcMasterPcba/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmasterpcba:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmasterpcba:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmasterpcba:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlavePcbaList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="pcbaSfid" label="ID"/>
        <el-table-column prop="pcbaParentSfid" label="父ID"/>
        <el-table-column prop="pcbaEcNo" label="设变No."/>
        <el-table-column prop="pcbaModel" label="机种"/>
        <el-table-column prop="pcbaItem" label="物料"/>
        <el-table-column prop="pcbaImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.pcbaImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="pcbaEnteredDate" label="制二登入日期"/>
        <el-table-column prop="pcbaLine" label="Pcba班组">
          <template #default="scope">
            <dict-tag :options=" options.sql_line_list " :value="scope.row.pcbaLine"  />
          </template>
        </el-table-column>
        <el-table-column prop="pcbaPutLot" label="Pcba批次"/>
        <el-table-column prop="pcbaNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">Pcba设变</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlavePcba">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlavePcba">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlavePcbaList" :row-class-name="rowPpEcSlavePcbaIndex" @selection-change="handlePpEcSlavePcbaSelectionChange" ref="PpEcSlavePcbaRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="pcbaSfid">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="pcbaEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="pcbaModel">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="pcbaItem">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaItem" :placeholder="$t('btn.enterPrefix')+'物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="实施标记" prop="pcbaImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.pcbaImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="制二登入日期" align="center" prop="pcbaEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.pcbaEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="Pcba班组" prop="pcbaLine">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.pcbaLine" :placeholder="$t('btn.enterPrefix')+'Pcba班组'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="Pcba批次" align="center" prop="pcbaPutLot">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaPutLot" :placeholder="$t('btn.enterPrefix')+'Pcba批次'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="说明" align="center" prop="pcbaNote">
            <template #default="scope">
              <el-input v-model="scope.row.pcbaNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmasterpcba">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterPcba,
 addPpEcMasterPcba, delPpEcMasterPcba, 
 updatePpEcMasterPcba,getPpEcMasterPcba, 
 } 
from '@/api/logistics/ppecmasterpcba.js'
...
//API获取从制二/pp_ec_master_pcba表记录数据
function getList(){
  ...
  loading.value = true
  listPpEcMasterPcba(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'制二'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterPcba(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '制二'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlavePcbaList.value = res.data.ppEcSlavePcbaNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlavePcbaNav = ppEcSlavePcbaList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterPcba(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterPcba(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<制二.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterPcba/export', { ...queryParams })
    })
}

/*********************Pcba设变子表信息*************************/
const ppEcSlavePcbaList = ref([])
const checkedPpEcSlavePcba = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** Pcba设变序号 */
function rowPpEcSlavePcbaIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** Pcba设变添加按钮操作 */
function handleAddPpEcSlavePcba() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.pcbaSfid = null;
  //obj.pcbaEcNo = null;
  //obj.pcbaModel = null;
  //obj.pcbaItem = null;
  //obj.pcbaImplStated = null;
  //obj.pcbaEnteredDate = null;
  //obj.pcbaLine = null;
  //obj.pcbaPutLot = null;
  //obj.pcbaNote = null;
  //obj.remark = null;
  ...
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlavePcbaList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlavePcbaSelectionChange(selection) {
  checkedPpEcSlavePcba.value = selection.map(item => item.index)
}

/** Pcba设变删除按钮操作 */
function handleDeletePpEcSlavePcba() {
  if(checkedPpEcSlavePcba.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的Pcba设变数据')
  } else {
    const PpEcSlavePcbas = ppEcSlavePcbaList.value;
    const checkedPpEcSlavePcbas = checkedPpEcSlavePcba.value;
    ppEcSlavePcbaList.value = PpEcSlavePcbas.filter(function(item) {
      return checkedPpEcSlavePcbas.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterPcba(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlavePcbaList.value = data.ppEcSlavePcbaNav
    }
  })
}
