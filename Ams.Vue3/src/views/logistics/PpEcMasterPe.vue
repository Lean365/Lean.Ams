<!--
 * @Descripttion: 制技/pp_ec_master_pe
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecmasterpe:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecmasterpe:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecmasterpe:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecmasterpe:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcMasterPe/importTemplate"
                  importUrl="/Logistics/PpEcMasterPe/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecmasterpe:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecmasterpe:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecmasterpe:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="drawer" size="65%" direction="rtl">
      <el-table :data="ppEcSlavePeList" header-row-class-name="text-navy">
        <el-table-column :label="$t('layout.indexNo')" type="index" width="80" />
        <el-table-column prop="peSfid" label="ID"/>
        <el-table-column prop="peParentSfid" label="父ID"/>
        <el-table-column prop="peEcNo" label="设变No."/>
        <el-table-column prop="peModel" label="机种"/>
        <el-table-column prop="peItem" label="物料"/>
        <el-table-column prop="peSubItem" label="子物料"/>
        <el-table-column prop="peSopStated" label="SOP">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.peSopStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="peImplStated" label="实施标记">
          <template #default="scope">
            <dict-tag :options=" options.sys_flag_list " :value="scope.row.peImplStated"  />
          </template>
        </el-table-column>
        <el-table-column prop="peEnteredDate" label="SOP确认日期"/>
        <el-table-column prop="peDeptSop" label="SOP部门">
          <template #default="scope">
            <dict-tag :options=" options.sql_dept_list " :value="scope.row.peDeptSop"  />
          </template>
        </el-table-column>
        <el-table-column prop="peNote" label="说明"/>
    ...
    <!-- 子表信息 -->
        <el-divider content-position="center">Pe设变</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="btn-add" icon="Plus" @click="handleAddPpEcSlavePe">{{ $t('btn.add') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="btn-delete" icon="Delete" @click="handleDeletePpEcSlavePe">{{ $t('btn.delete') }}</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table :data="ppEcSlavePeList" :row-class-name="rowPpEcSlavePeIndex" @selection-change="handlePpEcSlavePeSelectionChange" ref="PpEcSlavePeRef">
          <el-table-column type="selection" width="40" align="center" />
          <el-table-column :label="$t('layout.indexNo')" align="center" prop="index" width="50"/>
          <el-table-column label="ID" align="center" prop="peSfid">
            <template #default="scope">
              <el-input v-model="scope.row.peSfid" :placeholder="$t('btn.enterPrefix')+'ID'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="设变No." align="center" prop="peEcNo">
            <template #default="scope">
              <el-input v-model="scope.row.peEcNo" :placeholder="$t('btn.enterPrefix')+'设变No.'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="机种" align="center" prop="peModel">
            <template #default="scope">
              <el-input v-model="scope.row.peModel" :placeholder="$t('btn.enterPrefix')+'机种'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="物料" align="center" prop="peItem">
            <template #default="scope">
              <el-input v-model="scope.row.peItem" :placeholder="$t('btn.enterPrefix')+'物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="子物料" align="center" prop="peSubItem">
            <template #default="scope">
              <el-input v-model="scope.row.peSubItem" :placeholder="$t('btn.enterPrefix')+'子物料'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
          <el-table-column label="SOP" prop="peSopStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.peSopStated" :placeholder="$t('btn.enterPrefix')+'SOP'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="实施标记" prop="peImplStated">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.peImplStated" :placeholder="$t('btn.enterPrefix')+'实施标记'+$t('btn.enterSuffix')">
                <el-option
                  v-for="item in options.sys_flag_list" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
          ...
          <el-table-column label="SOP确认日期" align="center" prop="peEnteredDate">
            <template #default="scope">
              <el-date-picker clearable v-model="scope.row.peEnteredDate" type="date" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </template>
          </el-table-column>
          <el-table-column label="SOP部门" prop="peDeptSop">
            <template #default="scope">
              <el-select filterable clearable  v-model="scope.row.peDeptSop" :placeholder="$t('btn.enterPrefix')+'SOP部门'+$t('btn.enterSuffix')">
                <el-option
          ...
          <el-table-column label="说明" align="center" prop="peNote">
            <template #default="scope">
              <el-input v-model="scope.row.peNote" :placeholder="$t('btn.enterPrefix')+'说明'+$t('btn.enterSuffix')" />
            </template>
          </el-table-column>
...
<script setup name="ppecmasterpe">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcMasterPe,
 addPpEcMasterPe, delPpEcMasterPe, 
 updatePpEcMasterPe,getPpEcMasterPe, 
 } 
from '@/api/logistics/ppecmasterpe.js'
...
function getList(){
  ...
  loading.value = true
  listPpEcMasterPe(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'制技'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcMasterPe(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '制技'
      opertype.value = 2

      form.value = {
        ...data,
      }
      ppEcSlavePeList.value = res.data.ppEcSlavePeNav
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.ppEcSlavePeNav = ppEcSlavePeList.value
      if (form.value.ecmSFID != undefined && opertype.value === 2) {
        updatePpEcMasterPe(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpEcMasterPe(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<制技.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcMasterPe/export', { ...queryParams })
    })
}

/*********************Pe设变子表信息*************************/
const ppEcSlavePeList = ref([])
const checkedPpEcSlavePe = ref([])
const fullScreen = ref(false)
const drawer = ref(false)

/** Pe设变序号 */
function rowPpEcSlavePeIndex({ row, rowIndex }) {
  row.index = rowIndex + 1;
}

/** Pe设变添加按钮操作 */
function handleAddPpEcSlavePe() {
  let obj = {};
  //下面的代码自己设置默认值
  //obj.peSfid = null;
  //obj.peEcNo = null;
  ...
  //obj.peItem = null;
  //obj.peSubItem = null;
  //obj.peSopStated = null;
  //obj.peImplStated = null;
  //obj.peEnteredDate = null;
  //obj.peDeptSop = null;
  //obj.peNote = null;
  //obj.remark = null;
  //obj.createBy = null;
  ...
  //obj.updateBy = null;
  //obj.updateTime = null;
  ppEcSlavePeList.value.push(obj);
}

/** 复选框选中数据 */
function handlePpEcSlavePeSelectionChange(selection) {
  checkedPpEcSlavePe.value = selection.map(item => item.index)
}

/** Pe设变删除按钮操作 */
function handleDeletePpEcSlavePe() {
  if(checkedPpEcSlavePe.value.length == 0){
    proxy.$modal.msgError('请先选择要删除的Pe设变数据')
  } else {
    const PpEcSlavePes = ppEcSlavePeList.value;
    const checkedPpEcSlavePes = checkedPpEcSlavePe.value;
    ppEcSlavePeList.value = PpEcSlavePes.filter(function(item) {
      return checkedPpEcSlavePes.indexOf(item.index) == -1
    });
  }
}
...
function rowClick(row) {
  ...
  getPpEcMasterPe(id).then((res) => {
    ...
    if (code == 200) {
      drawer.value = true
      ppEcSlavePeList.value = data.ppEcSlavePeNav
    }
  })
}
