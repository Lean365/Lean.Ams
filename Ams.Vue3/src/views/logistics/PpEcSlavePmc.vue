  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="输入日期">
        <el-date-picker
          v-model="dateRangePmcEnteredDate" 
          type="datetimerange"
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:ecslavepmc:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:ecslavepmc:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:ecslavepmc:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:ecslavepmc:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpEcSlavePmc/importTemplate"
                  importUrl="/Logistics/PpEcSlavePmc/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:ecslavepmc:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:ecslavepmc:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:ecslavepmc:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="输入日期" prop="pmcEnteredDate">
              <el-date-picker v-model="form.pmcEnteredDate" type="datetime" :teleported="false" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </el-form-item>
          </el-col>
...
<script setup name="ppecslavepmc">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpEcSlavePmc,
 addPpEcSlavePmc, delPpEcSlavePmc, 
 updatePpEcSlavePmc,getPpEcSlavePmc, 
 } 
from '@/api/logistics/ppecslavepmc.js'
...
//是否查询（1是）
  pmcItem: undefined,
//是否查询（1是）
  pmcImplStated: undefined,
//是否查询（1是）
  pmcEnteredDate: undefined,
//是否查询（1是）
  pmcPreLot: undefined,
//是否查询（1是）
  pmcDisposal: undefined,
})
//字段显示控制
const columns = ref([
...
// 输入日期时间范围
const dateRangePmcEnteredDate = ref([])
...
//API获取从Pmc设变/pp_ec_slave_pmc表记录数据
function getList(){
  proxy.addDateRange(queryParams, dateRangePmcEnteredDate.value, 'PmcEnteredDate');
  loading.value = true
  listPpEcSlavePmc(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'Pmc设变'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getPpEcSlavePmc(id).then((res) => {
    ...
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ 'Pmc设变'
      opertype.value = 2
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.pmcSfId != undefined && opertype.value === 2) {
        ...
        addPpEcSlavePmc(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<Pmc设变.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/PpEcSlavePmc/export', { ...queryParams })
    })
}
