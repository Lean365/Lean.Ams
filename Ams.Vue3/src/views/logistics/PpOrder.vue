  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="状态" prop="isState">
        <el-select filterable clearable   v-model="queryParams.isState" :placeholder="$t('btn.select')+'状态'">
          <el-option v-for="item in   options.app_mo_state " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>          
          </el-option>
        </el-select>
    ...
    <el-table border height="650"
      ...
      <el-table-column prop="moOrderItem" label="物料" align="center" v-if="columns.showColumn('moOrderItem')">
        <template #default="scope">
          <dict-tag :options=" options.sql_moc_item " :value="scope.row.moOrderItem"  />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="isState" label="状态" align="center" v-if="columns.showColumn('isState')">
        <template #default="scope">
          <dict-tag :options=" options.app_mo_state " :value="scope.row.isState"  />
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="auto">
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="状态" prop="isState">
              <el-select filterable clearable   v-model="form.isState"  :placeholder="$t('btn.select')+'状态'">
                <el-option
                  v-for="item in  options.app_mo_state" 
                  :key="item.dictValue" 
                  :label="item.dictLabel" 
...
const state = reactive({
  ...
  rules: {
    ...
    isState: [{ required: true, message: "状态不能为空", trigger: "change"    , type: "number"  }],
  },
  options: {
