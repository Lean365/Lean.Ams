<!--
 * @Descripttion: 组立明细/pp_output_assy_slv
  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="生产时段" prop="mmb002">
        <el-input v-model="queryParams.mmb002" :placeholder="$t('btn.enterSearchPrefix')+'生产时段'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
        </el-col>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['pp:outputassyslv:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['pp:outputassyslv:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['pp:outputassyslv:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['pp:outputassyslv:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/PpOutputAssySlv/importTemplate"
                  importUrl="/Logistics/PpOutputAssySlv/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['pp:outputassyslv:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:outputassyslv:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:outputassyslv:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="ppoutputassyslv">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listPpOutputAssySlv,
 addPpOutputAssySlv, delPpOutputAssySlv, 
 updatePpOutputAssySlv,getPpOutputAssySlv, 
 } 
from '@/api/logistics/ppoutputassyslv.js'
...
//API获取从组立明细/pp_output_assy_slv表记录数据
function getList(){
  loading.value = true
  listPpOutputAssySlv(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleUpdate(row) {
  ...
  getPpOutputAssySlv(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.id != undefined && opertype.value === 2) {
        updatePpOutputAssySlv(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addPpOutputAssySlv(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/Logistics/PpOutputAssySlv/export', { ...queryParams })
    })
}
