...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button type="add" v-hasPermi="['pp:outputslave:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="edit" :disabled="single" v-hasPermi="['pp:outputslave:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="deletebatch" :disabled="multiple" v-hasPermi="['pp:outputslave:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="logistics/PpOutputSlave/importTemplate"
                  importUrl="/logistics/PpOutputSlave/importData"
      ...
      <el-col :span="1.5">
        <el-button type="export" plain icon="download" @click="handleExport" v-hasPermi="['pp:outputslave:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="650"
      ...
      <el-table-column prop="posUnfinishedReasons" label="未达成原因" align="center" v-if="columns.showColumn('posUnfinishedReasons')">
        <template #default="scope">
          <dict-tag :options=" options.sql_non_conf " :value="scope.row.posUnfinishedReasons" split="," />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button type="edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['pp:outputslave:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button type="delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['pp:outputslave:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="父SFID" prop="posParentSfid">
              <el-input-number v-model.number="form.posParentSfid" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'父SFID'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="生产时段" prop="posProductionTime">
              <el-select filterable clearable   v-model="form.posProductionTime"  :placeholder="$t('btn.selectPrefix')+'生产时段'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="实际产能" prop="posRealOutput">
              <el-input-number v-model.number="form.posRealOutput" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'实际产能'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
            
          <el-col :lg="12">
            <el-form-item label="停线时间" prop="posDownTime">
              <el-input-number v-model.number="form.posDownTime" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'停线时间'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="未达成原因" prop="posUnfinishedReasons">
              <el-select filterable clearable multiple collapse-tags collapse-tags-tooltip  v-model="form.posUnfinishedReasonsChecked"  :placeholder="$t('btn.selectPrefix')+'未达成原因'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="未达成说明" prop="posUnfinishedDescription">
              <el-input v-model="form.posUnfinishedDescription" :placeholder="$t('btn.enterPrefix')+'未达成说明'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
            
          <el-col :lg="12">
            <el-form-item label="实际工数" prop="posRealWorkhours">
              <el-input-number v-model.number="form.posRealWorkhours" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'实际工数'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="投入工数" prop="posInputsWorkhours">
              <el-input-number v-model.number="form.posInputsWorkhours" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'投入工数'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="工时差异" prop="posRealWorkinghoursDiff">
              <el-input-number v-model.number="form.posRealWorkinghoursDiff" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'工时差异'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="产能差异" prop="posRealOutputDiff">
              <el-input-number v-model.number="form.posRealOutputDiff" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'产能差异'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="达成率" prop="posAchievedRate">
              <el-input-number v-model.number="form.posAchievedRate" :controls="true" controls-position="right" :placeholder="$t('btn.enterPrefix')+'达成率'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>
...
const state = reactive({
  ...
  rules: {
    ...
    posRealOutput: [{ required: true, message: "实际产能"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    posDownTime: [{ required: true, message: "停线时间"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
    posRealWorkhours: [{ required: true, message: "实际工数"+proxy.$t('btn.isEmpty'), trigger: "blur"    , type: "number"  }],
  },
  options: {
  ...
  getPpOutputSlave(id).then((res) => {
    ...
    if (code == 200) {
      ...
      form.value = {
        ...
        posUnfinishedReasonsChecked: data.posUnfinishedReasons ? data.posUnfinishedReasons.split(',') : [],
      }
    }
  })
}
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      form.value.posUnfinishedReasons = form.value.posUnfinishedReasonsChecked.toString();
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<oph从表.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/logistics/PpOutputSlave/export', { ...queryParams })
    })
}
