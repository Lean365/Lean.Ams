  ...
  <div>
    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent label-width="auto">
      ...
      <el-form-item label="外部客诉No." prop="qmcsIssuesNo">
        <el-input v-model="queryParams.qmcsIssuesNo" :placeholder="$t('btn.enterSearchPrefix')+'外部客诉No.'+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item label="班组" prop="qmcsLine">
        <el-select filterable clearable   v-model="queryParams.qmcsLine" :placeholder="$t('btn.selectSearchPrefix')+'班组'+$t('btn.selectSearchSuffix')">
      ...
      <el-form-item label="处理日期">
        <el-date-picker
          v-model="dateRangeQmcsProcessingDate" 
          type="datetimerange"
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['qm:complaintsslave:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['qm:complaintsslave:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['qm:complaintsslave:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['qm:complaintsslave:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Logistics/QmComplaintsSlave/importTemplate"
                  importUrl="/Logistics/QmComplaintsSlave/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['qm:complaintsslave:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['qm:complaintsslave:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['qm:complaintsslave:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="SFID" prop="qmcsSFID">
              <el-input v-model.number="form.qmcsSFID" :placeholder="$t('btn.enterPrefix')+'SFID'+$t('btn.enterSuffix')" :disabled="opertype != 1"/>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="外部客诉No." prop="qmcsIssuesNo">
              <el-input v-model="form.qmcsIssuesNo" :placeholder="$t('btn.enterPrefix')+'外部客诉No.'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="班组" prop="qmcsLine">
              <el-select filterable clearable   v-model="form.qmcsLine"  :placeholder="$t('btn.selectPrefix')+'班组'+$t('btn.selectSuffix')">
                <el-option
          ...
          <el-col :lg="12">
            <el-form-item label="处理日期" prop="qmcsProcessingDate">
              <el-date-picker v-model="form.qmcsProcessingDate" type="datetime" :teleported="false" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="问题描述" prop="qmcsDescription">
              <el-input type="textarea" v-model="form.qmcsDescription" :placeholder="$t('btn.enterPrefix')+'问题描述'+$t('btn.enterSuffix')"/>
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="原因分析" prop="qmcsCauseAnalysis">
              <el-input type="textarea" v-model="form.qmcsCauseAnalysis" :placeholder="$t('btn.enterPrefix')+'原因分析'+$t('btn.enterSuffix')"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="24">
            <el-form-item label="工位" prop="qmcsStation">
              <el-input type="textarea" v-model="form.qmcsStation" :placeholder="$t('btn.enterPrefix')+'工位'+$t('btn.enterSuffix')"/>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="批次" prop="qmcsLot">
              <el-input v-model="form.qmcsLot" :placeholder="$t('btn.enterPrefix')+'批次'+$t('btn.enterSuffix')" />
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="改善对策" prop="qmcsMeasures">
              <el-input type="textarea" v-model="form.qmcsMeasures" :placeholder="$t('btn.enterPrefix')+'改善对策'+$t('btn.enterSuffix')"/>
            </el-form-item>
          </el-col>
...
<script setup name="qmcomplaintsslave">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listQmComplaintsSlave,
 addQmComplaintsSlave, delQmComplaintsSlave, 
 updateQmComplaintsSlave,getQmComplaintsSlave, 
 } 
from '@/api/logistics/qmcomplaintsslave.js'
...
function getList(){
  proxy.addDateRange(queryParams, dateRangeQmcsProcessingDate.value, 'QmcsProcessingDate');
  loading.value = true
  listQmComplaintsSlave(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function resetQuery(){
  // 处理日期时间范围
  dateRangeQmcsProcessingDate.value = []
...
const state = reactive({
  ...
  form: {},
  rules: {
    qmcsSFID: [{ required: true, message: "SFID"+proxy.$t('btn.isEmpty'), trigger: "blur" }],
    qmcsIssuesNo: [{ required: true, message: "外部客诉No."+proxy.$t('btn.isEmpty'), trigger: "blur"     }],
...
function handleUpdate(row) {
  ...
  getQmComplaintsSlave(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.qmcsSFID != undefined && opertype.value === 2) {
        updateQmComplaintsSlave(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addQmComplaintsSlave(form.value).then((res) => {
...
function handleDelete(row) {
  ...
  proxy
    ...
    .then(function () {
      return delQmComplaintsSlave(Ids)
    })
    .then(() => {
      getList()
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<从客诉管理.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Logistics/QmComplaintsSlave/export', { ...queryParams })
    })
}
