<!--
 * @Descripttion: 福利/routine_ehr_benefits
 * @Version: 1.0.0.0
...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button class="btn-add" v-hasPermi="['routine:ehrbenefits:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-edit" :disabled="single" v-hasPermi="['routine:ehrbenefits:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" :disabled="multiple" v-hasPermi="['routine:ehrbenefits:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['routine:ehrbenefits:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="Routine/RoutineEhrBenefits/importTemplate"
                  importUrl="/Routine/RoutineEhrBenefits/importData"
      ...
      <el-col :span="1.5">
        <el-button class="btn-export" plain icon="download" @click="handleExport" v-hasPermi="['routine:ehrbenefits:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table border height="600px"
      ...
      <el-table-column :label="$t('btn.operation')" width="160" align="center">
        <template #default="scope">
          <el-button-group>
          <el-button class="btn-edit" plain size="small" icon="edit" :title="$t('btn.edit')" v-hasPermi="['routine:ehrbenefits:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button class="btn-delete" plain size="small" icon="delete" :title="$t('btn.delete')" v-hasPermi="['routine:ehrbenefits:delete']" @click="handleDelete(scope.row)"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="routineehrbenefits">
  import '@/assets/styles/btn-custom.scss'
//后台操作函数
import { listRoutineEhrBenefits,
 addRoutineEhrBenefits, delRoutineEhrBenefits, 
 updateRoutineEhrBenefits,getRoutineEhrBenefits, 
 } 
from '@/api/routine/routineehrbenefits.js'
...
//API获取从福利/routine_ehr_benefits表记录数据
function getList(){
  loading.value = true
  listRoutineEhrBenefits(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleAdd() {
  reset();
  open.value = true
  title.value = proxy.$t('btn.add')+" "+'福利'
  opertype.value = 1
...
function handleUpdate(row) {
  ...
  getRoutineEhrBenefits(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
      title.value = proxy.$t('btn.edit')+" "+ '福利'
      opertype.value = 2
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.eeSFID != undefined && opertype.value === 2) {
        updateRoutineEhrBenefits(form.value).then((res) => {
         proxy.$modal.msgSuccess(proxy.$t('common.tipEditSucceed'))
          open.value = false
          getList()
        })
      } else {
        addRoutineEhrBenefits(form.value).then((res) => {
...
function handleDelete(row) {
  ...
  proxy
    ...
    .then(function () {
      return delRoutineEhrBenefits(Ids)
    })
    .then(() => {
      getList()
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.tipConfirmExport')+"<福利.xlsx>", proxy.$t('btn.export')+' '+proxy.$t('common.tip'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/Routine/RoutineEhrBenefits/export', { ...queryParams })
    })
}
