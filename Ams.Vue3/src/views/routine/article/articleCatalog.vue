<!--
 * @Descripttion: (文章目录/ArticleCatalog)
 * @version: (1.0)
 * @Author: (zr)
 * @Date: (2022-05-13)
 * @LastEditors: (zr)
    ...
    <el-table v-if="refreshTable" :data="dataList" v-loading="loading" ref="tableRef" border highlight-current-row
      ...
      <el-table-column prop="orderNum" :label="$t('btn.sort')" sortable align="center">
        <template #default="scope">
          ...
          <el-input :ref="setColumnsRef" v-show="editIndex == scope.row.categoryId" v-model="scope.row.orderNum"
...
<script setup name="ArticleCatalog">
  import {
    treelistArticleCatalog,
    addArticleCatalog,
    delArticleCatalog,
    updateArticleCatalog,
    getArticleCatalog,
    exportArticleCatalog,
    changeSort
  } from '@/api/routine/articlecatalog.js'
  ...
  function getList() {
    loading.value = true
    treelistArticleCatalog(queryParams).then((res) => {
      if (res.code == 200) {
