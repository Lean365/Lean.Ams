        ...
        <el-col :lg="12">
          <el-form-item prop="categoryId" label="选择圈子">
            <el-select v-model="form.categoryId" clearable>
              <el-option v-for="item in categoryOptions" :key="item.categoryId" :label="item.name"
        ...
        <el-col :lg="24">
          <el-form-item prop="commentSwitch" label="评论权限">
            <el-select clearable v-model="form.commentSwitch">
              <el-option v-for="item in options.sys_comment_permi" :label="item.dictLabel"
...
<script setup name="articlepublish">
  import { addArticle } from '@/api/routine/article.js'
  import { listArticleCatalog } from '@/api/routine/articlecatalog.js'
  import { listArticleTopic } from '@/api/routine/articletopic'
  ...
  function getCategoryTreeselect() {
    listArticleCatalog({ categoryType: 1, pageSize: 100 }).then((res) => {
      if (res.code == 200) {
