...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['tool:email:send']" plain icon="plus" @click="handleSendEmail"> 发送邮件
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" :disabled="multiple" v-hasPermi="['tool:email:send']" plain icon="upload"
          @click="handleSend">
          批量发送
        </el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['EmailItems:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border header-cell-class-name="el-table-header-cell"
      ...
      <el-table-column prop="fromEmail" label="发送邮箱" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fromEmail')" />
      <el-table-column prop="subject" label="邮件主题" :show-overflow-tooltip="true" v-if="columns.showColumn('subject')" />
      <el-table-column prop="toEmails" label="接收邮箱" :show-overflow-tooltip="true"
        v-if="columns.showColumn('toEmails')" />
      <el-table-column prop="emailContent" label="邮件内容" :show-overflow-tooltip="true"
        v-if="columns.showColumn('emailContent')" />
      ...
      <el-table-column label="操作" width="200">
        <template #default="scope">
          ...
          <el-button type="danger" size="small" icon="delete" text v-hasPermi="['EmailItems:delete']"
            @click="handleDelete(scope.row)">
            删除
          </el-button>
          <el-button type="primary" size="small" icon="upload" text v-hasPermi="['tool:email:send']"
            @click="handleSend(scope.row)" v-if="scope.row.isSend == 0">
            发送
          </el-button>
        </template>
      </el-table-column>
...
<script setup name="EmailItems">
  import { listEmailItems, sendEmail, delEmailItems } from '@/api/routine/emailitems.js'
  ...
  function getList() {
    ...
    loading.value = true
    listEmailItems(queryParams).then((res) => {
      const { code, data } = res
      if (code == 200) {
