  ...
  <div class="app-container">
    <el-form :model="queryParams" inline @submit.prevent ref="queryRef" v-show="searchToggle">
      <el-form-item prop="taskType">
        <el-select clearable v-model="queryParams.taskType" :placeholder="$t('btn.select')+$t('ptask.taskType')">
          <el-option v-for="item in options.taskTypeOptions" :key="item.dictValue" :label="item.dictLabel"
            :value="parseInt(item.dictValue)" />
        </el-select>
      </el-form-item>
      <el-form-item prop="name">
        <el-input v-model="queryParams.name" :placeholder="$t('btn.enter')+$t('ptask.name')" clearable
    ...
    <el-row class="mb8" :gutter="20">
      <el-col :span="1.5">
        <el-button v-hasPermi="['routine:task:add']" plain type="add" icon="plus" @click="handleCreate">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button plain v-hasPermi="['routine:task:query']" type="log" icon="operation" @click="onJobLogView()">
          {{ $t('btn.log') }}
        </el-button>
      </el-col>
    ...
    <el-table v-if="viewSwitch == 1" ref="tasks" v-loading="loading" :data="dataTasks" border row-key="id"
      ...
      <el-table-column :label="$t('btn.operation')" width="190" align="center">
        <template #default="scope">
          <el-button-group>
            <el-button plain type="view" size="small" icon="view" v-hasPermi="['routine:task:query']"
              @click="handleDetails(scope.row)" :title="$t('btn.details')">

            </el-button>
            <el-button plain type="log" size="small" icon="operation" v-hasPermi="['routine:task:query']"
                ...
                <el-dropdown-menu>
                  <div v-hasPermi="['routine:task:run']" v-if="scope.row.isStart">
                    <el-dropdown-item command="run">
                      <el-button type="start" plain size="small" icon="remove" :title="$t('btn.runOnce')">
                      </el-button>
                    </el-dropdown-item>
                  </div>
        ...
        <el-row>
          ...
          <el-col :lg="12">
            <el-form-item :label="$t('ptask.name')" maxlength="200" prop="name">
              <el-input v-model="form.name" :placeholder="$t('btn.enter')+$t('ptask.name')" />
            </el-form-item>
          </el-col>
          <el-col :lg="12">
            <el-form-item :label="$t('ptask.triggerType')" prop="triggerType">
              <el-select v-model="form.triggerType" :placeholder="$t('btn.select')+$t('ptask.triggerType')"
                style="width: 100%">
          ...
          <el-col :lg="12">
            <el-form-item :label="$t('ptask.taskGroup')" maxlength="200" prop="jobGroup">
              <el-select v-model="form.jobGroup" :placeholder="$t('btn.select')+$t('ptask.taskGroup')">
          ...
          <el-col :lg="24" v-if="form.taskType == 2">
            <el-form-item :label="$t('ptask.apiUrl')" prop="apiUrl">
              <el-input v-model="form.apiUrl" :placeholder="$t('btn.enter')+$t('ptask.apiUrl')">
                <template #prepend>
          ...
          <template v-if="form.taskType == 1">
            <el-col :lg="24">
              <el-form-item :label="$t('ptask.assemblyName')" maxlength="200" prop="assemblyName">
                <el-input v-model="form.assemblyName" :placeholder="$t('btn.enter')+$t('ptask.assemblyName')" />
              </el-form-item>
            </el-col>
            <el-col :lg="24">
              <el-form-item :label="$t('ptask.className')" maxlength="200" prop="className">
                <el-input v-model="form.className" :placeholder="$t('btn.enter')+$t('ptask.className')" />
              </el-form-item>
            </el-col>
          </template>
          ...
          <el-col :lg="24">
            <el-form-item :label="$t('ptask.taskParams')" prop="jobParams">
              ...
              <el-input v-model="form.jobParams" :placeholder="$t('btn.enter')+$t('ptask.taskParams')" />
            </el-form-item>
          </el-col>
          <el-col :lg="24" v-if="form.triggerType == 1">
            <el-form-item :label="$t('ptask.cron')" prop="cron">
              <el-input v-model="form.cron" :placeholder="$t('btn.enter')+$t('ptask.cron')">
                <template #append>
...
<script setup name="job">
  import { queryTasks, getTasks, createTasks, updateTasks, deleteTasks, startTasks, stopTasks, runTasks, exportTasks } from '@/api/routine/task'
  import { listJobLog } from '@/api/monitor/tasklog'
  ...
  function onJobLogView() {
    router.push({ path: 'monitor/TaskLog' })
  }
  /** 任务日志列表查询 */
