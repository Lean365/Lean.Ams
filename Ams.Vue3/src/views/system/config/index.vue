        ...
        <template #default="scope">
          <el-button-group>
            ...
            <el-button size="small" type="danger" plain icon="delete" @click="handleDelete(scope.row)"
              v-hasPermi="['system:config:delete']" :title="$t('btn.delete')">
            </el-button>
          </el-button-group>
        </template>
      </el-table-column>
...
<script setup name="config">
  ...
  function handleAdd() {
    reset()
    open.value = true
    title.value = proxy.$t('btn.add') + " " + proxy.$t('pconfig.config')
  }
  // 多选框选中数据
  ...
  function handleUpdate(row) {
    ...
    getConfig(configId).then((response) => {
      ...
      open.value = true
      title.value = proxy.$t('btn.edit') + " " + proxy.$t('pconfig.config')
    })
  }
  /** 提交按钮 */
  function submitForm() {
  ...
  //   refreshCache().then(() => {
  //     proxy.$modal.msgSuccess(proxy.$t('common.refreshSucceed'))
  //   })
  // }
