<template>
  <div class="app-container">
        ...
        <el-col :lg="24">
          <el-form-item :label="$t('pmenu.parentMenu')" prop="parentId">
            <el-cascader
              ...
              :placeholder="$t('btn.selectSearchPrefix') + $t('pmenu.parentMenu') + $t('btn.selectSearchSuffix')"
              clearable
          ...
          <el-form-item :label="$t('pmenu.menuName')" prop="menuName">
            <el-input
              ...
              :placeholder="$t('btn.enterSearchPrefix') + $t('pmenu.menuName') + $t('btn.enterSearchSuffix')"
              clearable
          ...
          <el-form-item :label="$t('common.tipIsStated')" prop="isStatus">
            <el-radio-group v-model="queryParams.isStatus">
              ...
              <el-radio v-for="dict in options.sys_normal_disable" :key="dict.dictValue" :value="parseInt(dict.dictValue)">{{
                dict.dictLabel
              }}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :label="$t('pmenu.isShow')" prop="visible">
            <el-radio-group v-model="queryParams.visible">
              <!-- <el-radio :value="-1">{{$t('common.all')}}</el-radio> -->
              <el-radio v-for="dict in options.sys_menu_show" :key="dict.dictValue" :value="dict.dictValue">{{ dict.dictLabel }}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-col>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button class="btn-add" plain icon="Plus" @click="handleAdd" v-hasPermi="['system:menu:add']">{{ $t('btn.add') }}</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <vxe-table
      ...
      :data="menuList">
      <vxe-column field="menuName" :title="$t('pmenu.menuName')" tree-node width="160" drag-sort> </vxe-column>
      ...
      <vxe-column field="menuType" :title="$t('pmenu.menuType')" align="center" width="120">
        <template #default="scope">
          <el-tag :disable-transitions="true" type="danger" v-if="scope.row.menuType == 'M' && scope.row.isFrame == 1">{{
            $t('pmenu.link')
          }}</el-tag>
          <el-tag :disable-transitions="true" v-else-if="scope.row.menuType == 'C'">{{ $t('pmenu.menu') }}</el-tag>
          <el-tag :disable-transitions="true" type="success" v-else-if="scope.row.menuType == 'M'">{{ $t('pmenu.directory') }}</el-tag>
          <el-tag :disable-transitions="true" type="warning" v-else-if="scope.row.menuType == 'F'">{{ $t('pmenu.button') }}</el-tag>
        </template>
      </vxe-column>
      <vxe-column field="sortingNum" :title="$t('pmenu.sort')" width="110" sortable align="center" v-if="columns.showColumn('sortingNum')">
        <template #default="scope">
          <span v-show="editIndex != scope.row.menuId" @click="editCurrRow(scope.row.menuId)">{{ scope.row.sortingNum }}</span>
          <el-input
            :ref="setColumnsRef"
            v-show="editIndex == scope.row.menuId"
            v-model="scope.row.sortingNum"
      ...
      <vxe-column :title="$t('common.tipCreateTime')" align="center" field="createTime" show-overflow v-if="columns.showColumn('createTime')">
        <template #default="scope">
      ...
      <vxe-column :title="$t('btn.operation')" align="center" width="140">
        <template #default="scope">
          <el-button-group>
            <el-button class="btn-add" plain size="small" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['system:menu:add']"></el-button>
            <el-button
              class="btn-edit"
              plain
              size="small"
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <div v-hasPermi="['system:menu:remove']" class="mt10">
                    <el-dropdown-item>
                      <el-button
                        class="btn-deletebatch"
                        plain
                        size="small"
                        icon="DeleteFilled"
                        type="danger"
                        @click="handleDeleteAll(scope.row)"
...
function handleDelete(row) {
  proxy
    .$confirm(
      proxy.$t('common.tipConfirmDel') + row.menuName + proxy.$t('common.tipConfirmDelDataitems'),
