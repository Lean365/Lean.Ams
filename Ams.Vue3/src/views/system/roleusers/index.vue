<template>
  <div class="app-container">
      ...
      <el-row :gutter="20">
        <el-col :lg="24">
          ...
          <el-form-item :label="$t('puser.userName')">
            <el-input v-model="roleUserQueryParams.userName"
              :placeholder="$t('btn.selectSearchPrefix')+$t('puser.userName')+$t('btn.selectSearchSuffix')" clearable
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button class="btn-deletebatch" plain icon="circle-close" @click="cancelAuthUserAll"
          v-hasPermi="['system:roleusers:remove']">
          {{ $t('btn.batch') }}{{ $t('btn.cancel') }}{{ $t('btn.authorize') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="btn-close" plain icon="close" @click="handleClose">{{ $t('btn.close') }}</el-button>
      </el-col>
    </el-row>
    ...
    <el-table ref="roleUserTableRef" v-loading="loading" :data="roleUserList"
      ...
      <el-table-column prop="userId" align="center" label="Id" width="150" />
      ...
      <el-table-column prop="isStatus" align="center" :label="$t('common.tipIsStated')" width="110">
        <template #default="scope">
          <dict-tag :options="statusOptions" :value="scope.row.isStatus" />
        </template>
      </el-table-column>
      <el-table-column prop="remark" :show-overflow-tooltip="true" align="center" :label="$t('common.tipRemarks')" />
      <el-table-column align="center" :label="$t('btn.operation')">
        <template #default="scope">
    ...
    <el-dialog :title="$t('btn.add')+' '+$t('puser.userName')" v-model="open" append-to-body @close="cancel">
      <el-form :inline="true" @submit.prevent>
        <el-form-item>
          <el-input v-model="userQueryParams.userName"
            :placeholder="$t('btn.selectSearchPrefix')+$t('puser.userName')+$t('btn.selectSearchSuffix')" clearable
      ...
      <el-row>
        <el-col>
          <el-table ref="userTable" v-loading="loadingUser" :data="dataUserTable"
            ...
            <el-table-column prop="userName" align="center" :label="$t('puser.userName')" width="150" />
            <el-table-column prop="nickName" align="center" :label="$t('puser.userNick')" width="150" />
            <el-table-column prop="isStatus" align="center" :label="$t('common.tipIsStated')">
              <template #default="scope">
                <dict-tag :options="statusOptions" :value="scope.row.isStatus" />
              </template>
            </el-table-column>
...
<script setup name="roleUsers">
  ...
  function cancelAuthUserAll() {
    ...
    proxy
      .$confirm(proxy.$t('common.tipConfirmDel') + delSelections.value.length + proxy.$t('common.tipConfirmDelDataitems'), proxy.$t('btn.cancel') + ' ' + proxy.$t('common.tip'), {
      ...
      .then(() => {
        deleteRoleUsers({
          ...
          if (response.code === 200) {
            proxy.$message({
              message: proxy.$t('common.tipDeleteSucceed'),
              type: 'success'
            })
            getRoleUser()
          }
  ...
  function handleSubmit() {
    ...
    createRoleUsers({
      ...
      if (response.code === 200) {
        proxy.$message({
          message: proxy.$t('common.tipAddSucceed'),
          type: 'success'
        })
        getRoleUser()
        open.value = false
