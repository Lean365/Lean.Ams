<template>
  <el-dialog :title="$t('btn.import')" v-model="visible" width="900px" top="5vh" append-to-body>
    <el-form ref="queryRef" :inline="true" :rules="rules" :model="queryParams">
      ...
      <el-form-item :label="$t('gencode.tableName')">
        <el-input v-model="queryParams.tableName" clearable
          :placeholder="$t('btn.enterSearchPrefix')+$t('gencode.tableName')+$t('btn.enterSearchSuffix')" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row>
      <el-table height="650px" ref="table" @row-click="clickRow" :data="dbTableList" highlight-current-row
        :row-key="getRowKey" @selection-change="handleSelectionChange">
        <el-table-column type="selection" :reserve-selection="true" width="55"></el-table-column>
        <el-table-column prop="name" :label="$t('gencode.tableName')" sortable="custom" width="380" />
        <el-table-column prop="description" :label="$t('gencode.tableComment')" />
      </el-table>
    ...
    <template #footer>
      ...
      <el-button type="primary" :disabled="tables.length <= 0"
        @click="handleImportTable">{{$t('btn.submit')}}</el-button>
    </template>
  </el-dialog>
</template>
...
<script setup>
  ...
  const rules = reactive({
    dbName: [{ required: true, message: proxy.$t('btn.select') + proxy.$t('gencode.dbName'), trigger: 'blur' }]
  })
