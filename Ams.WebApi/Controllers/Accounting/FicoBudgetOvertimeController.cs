...
namespace Ams.WebApi.Controllers.Accounting
{
    ...
    public class FicoBudgetOvertimeController : BaseController
    {
        ...
        public IActionResult GetFicoBudgetOvertime(long FboSFID)
        {
            var response = _FicoBudgetOvertimeService.GetInfo(FboSFID);
        ...
        public IActionResult AddFicoBudgetOvertime([FromBody] FicoBudgetOvertimeDto parm)
        {
           // 校验输入项目唯一性
            ...
            if (UserConstants.NOT_UNIQUE.Equals(_FicoBudgetOvertimeService.CheckInputUnique(parm.FboSFID.ToString())))
            {
