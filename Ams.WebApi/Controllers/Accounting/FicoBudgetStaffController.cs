...
namespace Ams.WebApi.Controllers.Accounting
{
    ...
    public class FicoBudgetStaffController : BaseController
    {
        ...
        public IActionResult GetFicoBudgetStaff(long FbsSFID)
        {
            var response = _FicoBudgetStaffService.GetInfo(FbsSFID);
        ...
        public IActionResult AddFicoBudgetStaff([FromBody] FicoBudgetStaffDto parm)
        {
           // 校验输入项目唯一性
            ...
            if (UserConstants.NOT_UNIQUE.Equals(_FicoBudgetStaffService.CheckInputUnique(parm.FbsSFID.ToString())))
            {
