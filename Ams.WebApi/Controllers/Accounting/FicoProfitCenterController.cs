...
namespace Ams.WebApi.Controllers.Accounting
{
    ...
    public class FicoProfitCenterController : BaseController
    {
        ...
        public IActionResult GetFicoProfitCenter(long Id)
        {
            var response = _FicoProfitCenterService.GetInfo(Id);
        ...
        [Log(Title = "利润中心", BusinessType = BusinessType.ADD)]
        public IActionResult AddFicoProfitCenter([FromBody] FicoProfitCenterDto parm)
        {
           // 校验输入项目唯一性

            if (UserConstants.NOT_UNIQUE.Equals(_FicoProfitCenterService.CheckInputUnique(parm.Id.ToString())))
            {
                return ToResponse(ApiResult.Error($"新增利润中心 '{parm.Id}'失败(Add failed)，输入的利润中心已存在(The entered already exists)"));
            }
        ...
        [Log(Title = "利润中心", BusinessType = BusinessType.EDIT)]
        ...
        /// </summary>
        /// <returns></returns>
        [Log(Title = "利润中心导出", BusinessType = BusinessType.EXPORT, IsSaveResponseData = false)]
        [HttpGet("export")]
