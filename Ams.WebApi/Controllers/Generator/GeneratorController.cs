...
namespace Ams.WebApi.Controllers.Generator
{
    ...
    {
        private readonly GeneratorService _GeneratorService = new GeneratorService();
        ...
        {
            var dbList = _GeneratorService.GetAllDataBases();
        ...
        {
            List<DbTableInfo> list = _GeneratorService.GetAllTables(dbName, tableName, pager);
        ...
        public IActionResult ImportTableSave([FromBody] ImportGeneratorTableDto dto)
        {
            ...
            {
                List<GeneratorOracleSeq> seqs = new();
                GeneratorInitTableDto GeneratorInitTableDto = new()
                {
                    DbName = dto.DbName,
                ...
                GenTable genTable = GeneratorTool.InitTable(GeneratorInitTableDto);
                genTable.TableId = GenTableService.ImportGenTable(genTable);
                if (OptionsSetting.CodeGenDbConfig.DbType == 3)
                {
                    seqs = _GeneratorService.GetAllGeneratorOracleSeqs(table.Name);
                }
                if (genTable.TableId > 0)
                {
                    //保存列信息
                    List<DbColumnInfo> dbColumnInfos = _GeneratorService.GetColumnInfo(dto.DbName, table.Name);
                    List<GenTableColumn> genTableColumns = GeneratorTool.InitGenTableColumn(genTable, dbColumnInfos, seqs, OptionsSetting.CodeGen);
        ...
        public IActionResult Preview([FromQuery] GeneratorDto dto, [FromRoute] int tableId = 0)
        {
            dto.TableId = tableId;
        ...
        public IActionResult CodeGenerate([FromBody] GeneratorDto dto)
        {
            ...
            {
                SysMenuService.AddSysMenu(dto.GenTable, dto.GeneratorReplaceDto.PermissionPrefix, dto.GeneratorReplaceDto.ShowBtnEdit, dto.GeneratorReplaceDto.ShowBtnExport, dto.GeneratorReplaceDto.ShowBtnImport);
            }
        ...
        public IActionResult SynchDb(string tableName, long tableId = 0)
        {
            ...
            List<DbColumnInfo> dbColumnInfos = _GeneratorService.GetColumnInfo(table.DbName, tableName);
            List<GenTableColumn> dbTableColumns = GeneratorTool.InitGenTableColumn(table, dbColumnInfos, codeGen: codeGen);
