...
namespace Ams.WebApi.Controllers.Logistics
{
    ...
    public class MmMbewController : BaseController
    {
        ...
        public IActionResult GetMmMbew(long Id)
        {
            var response = _MmMbewService.GetInfo(Id);
        ...
        [Log(Title = "物料评估", BusinessType = BusinessType.ADD)]
        public IActionResult AddMmMbew([FromBody] MmMbewDto parm)
        {
           // 校验输入项目唯一性

            if (UserConstants.NOT_UNIQUE.Equals(_MmMbewService.CheckInputUnique(parm.Id.ToString())))
            {
                return ToResponse(ApiResult.Error($"新增物料评估 '{parm.Id}'失败(Add failed)，输入的物料评估已存在(The entered already exists)"));
            }
        ...
        [Log(Title = "物料评估", BusinessType = BusinessType.EDIT)]
        ...
        /// </summary>
        /// <returns></returns>
        [Log(Title = "物料评估导出", BusinessType = BusinessType.EXPORT, IsSaveResponseData = false)]
        [HttpGet("export")]
