    ...
    public class PpEcMasterController : BaseController
    {
        ...
        public IActionResult AddPpEcMaster([FromBody] PpEcMasterDto parm)
        {
           // 校验输入项目唯一性
            ...
            {
                return ToResponse(ApiResult.Error($"新增主设变 '{parm.EmSFID}'失败(New failed)，输入的主设变已存在(The entered already exists)"));
            }
        ...
        /// <returns></returns>
        [HttpPut]
        [ActionPermissionFilter(Permission = "pp:ecmaster:update")]
        ...
        {
            var result = DownloadImportTemplate(new List<PpEcMasterImportTpl>() { }, "pp_ec_master_tpl");
            return ExportExcel(result.Item2, result.Item1);
        }
