    ...
    public class PpEcSlaveController : BaseController
    {
        ...
        public IActionResult AddPpEcSlave([FromBody] PpEcSlaveDto parm)
        {
           // 校验输入项目唯一性
            ...
            {
                return ToResponse(ApiResult.Error($"新增从设变 '{parm.EsSFID}'失败(New failed)，输入的从设变已存在(The entered already exists)"));
            }
        ...
        /// <returns></returns>
        [HttpPut]
        [ActionPermissionFilter(Permission = "pp:ecslave:update")]
        ...
        {
            var result = DownloadImportTemplate(new List<PpEcSlaveImportTpl>() { }, "pp_ec_slave_tpl");
            return ExportExcel(result.Item2, result.Item1);
        }
