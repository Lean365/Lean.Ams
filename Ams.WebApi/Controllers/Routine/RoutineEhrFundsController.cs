...
namespace Ams.WebApi.Controllers.Routine
{
    ...
    public class RoutineEhrFundsController : BaseController
    {
        ...
        public IActionResult GetRoutineEhrFunds(long Id)
        {
            var response = _RoutineEhrFundsService.GetInfo(Id);
        ...
        [Log(Title = "公积金", BusinessType = BusinessType.ADD)]
        public IActionResult AddRoutineEhrFunds([FromBody] RoutineEhrFundsDto parm)
        {
           // 校验输入项目唯一性

            if (UserConstants.NOT_UNIQUE.Equals(_RoutineEhrFundsService.CheckInputUnique(parm.Id.ToString())))
            {
                return ToResponse(ApiResult.Error($"新增公积金 '{parm.Id}'失败(Add failed)，输入的公积金已存在(The entered already exists)"));
            }
        ...
        [Log(Title = "公积金", BusinessType = BusinessType.EDIT)]
        ...
        /// </summary>
        /// <returns></returns>
        [Log(Title = "公积金导出", BusinessType = BusinessType.EXPORT, IsSaveResponseData = false)]
        [HttpGet("export")]
