...
namespace Ams.WebApi.Controllers.Routine
{
    ...
    public class RoutineEhrSocialSecurityController : BaseController
    {
        ...
        public IActionResult GetRoutineEhrSocialSecurity(long Id)
        {
            var response = _RoutineEhrSocialSecurityService.GetInfo(Id);
        ...
        [Log(Title = "社保", BusinessType = BusinessType.ADD)]
        public IActionResult AddRoutineEhrSocialSecurity([FromBody] RoutineEhrSocialSecurityDto parm)
        {
           // 校验输入项目唯一性

            if (UserConstants.NOT_UNIQUE.Equals(_RoutineEhrSocialSecurityService.CheckInputUnique(parm.Id.ToString())))
            {
                return ToResponse(ApiResult.Error($"新增社保 '{parm.Id}'失败(Add failed)，输入的社保已存在(The entered already exists)"));
            }
        ...
        [Log(Title = "社保", BusinessType = BusinessType.EDIT)]
        ...
        /// </summary>
        /// <returns></returns>
        [Log(Title = "社保导出", BusinessType = BusinessType.EXPORT, IsSaveResponseData = false)]
        [HttpGet("export")]
