...
namespace Ams.WebApi.Controllers.Routine
{
    ...
    public class RoutineEhrTrainingController : BaseController
    {
        ...
        public IActionResult GetRoutineEhrTraining(long EeSfId)
        {
            var response = _RoutineEhrTrainingService.GetInfo(EeSfId);
        ...
        public IActionResult AddRoutineEhrTraining([FromBody] RoutineEhrTrainingDto parm)
        {
           // 校验输入项目唯一性
            ...
            if (UserConstants.NOT_UNIQUE.Equals(_RoutineEhrTrainingService.CheckInputUnique(parm.EeSfId.ToString())))
            {
                return ToResponse(ApiResult.Error($"新增培训 '{parm.EeSfId}'失败(Add failed)，输入的培训已存在(The entered already exists)"));
            }
