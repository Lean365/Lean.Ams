...
namespace Ams.WebApi.Controllers.System
{
    ...
    public class SysLoginController : BaseController
    {
        ...
        {
            if (loginBody == null) { throw new CustomizeException("请求参数错误"); }
            ...
            LogLoginService.CheckLockUser(loginBody.Username);
            string location = HttpContextExtension.GetIpInfo(loginBody.LoginIP);
            var user = LogLoginService.Login(loginBody, new LogLogin() { LoginLocation = location });
        ...
        public IActionResult VerifyScan([FromBody] ScanDto dto)
        {
            ...
            return SUCCESS(new { IsState, token });
        }
        ...
        public IActionResult ScanLogin([FromBody] ScanDto dto)
        {
            ...
            LogLoginService.CheckLockUser(name);
        ...
        public IActionResult CheckMobile([FromBody] PhoneLoginDto dto)
        {
            ...
            var info = sysUserService.GetFirst(f => f.Phonenumber == dto.PhoneNum) ?? throw new CustomizeException(ResultCode.CUSTOM_ERROR, "该手机号不存在", false);

            var smsCode = RandomHelper.GenerateNum(6);
            var smsContent = $"验证码{smsCode}（随机验证码）,有效期10分钟。";
            //TODO 发送短息验证码,1分钟内允许一次
            LogSmsService.AddLogSms(new Kernel.Model.Monitor.LogSms()
            {
                Userid = info.UserId,
        ...
        public IActionResult PhoneLogin([FromBody] PhoneLoginDto loginBody)
        {
            ...
            var info = sysUserService.GetFirst(f => f.Phonenumber == loginBody.PhoneNum) ?? throw new CustomizeException(ResultCode.CUSTOM_ERROR, "该手机号不存在", false);
            LogLoginService.CheckLockUser(info.UserName);
            string location = HttpContextExtension.GetIpInfo(loginBody.LoginIP);
            var user = LogLoginService.PhoneLogin(loginBody, new LogLogin() { LoginLocation = location }, info);
